 ...
 * σε όποιον δεν ξέρει τη γεωμετρία μας· η λέξη λέει τι ΒΙΩΝΕΙ κανείς — και από 28/08 είναι το
 * πάνω σκαλί μιας κλίμακας που ξεκινά στα 4 Μπφ.) Είναι ο ΙΔΙΟΣ κανόνας
 ...
 * `partial`), στα 4 «φυσάει αρκετά», στα 5 «φυσάει πολύ», από 6 «φυσάει δυνατά».
 ...
 *
 * ⚠️ ΑΠΟ 4 ΜΠΦ ΤΟ ΠΛΑΚΙΔΙΟ ΛΕΕΙ ΤΟ ΙΔΙΟ ΠΡΑΓΜΑ ΜΕ ΤΗΝ ΚΑΡΤΑ, ΜΕ ΡΗΜΑ ΑΝΤΙ ΓΙΑ ΟΥΣΙΑΣΤΙΚΟ
 * (Μίλτος, 28/08/2026 — screenshot Λέσβου, και η εντολή που ήρθε μετά: «θέλω ό,τι λέει η
 * εξωτερική κάρτα να συμβαδίζει με το μέσα»).
 *
 * ΤΙ ΕΛΕΓΕ Η ΟΘΟΝΗ. Το πλακίδιο: «Άνεμος · 4–5 Μπφ · Β · απάνεμη». Η γραμμή δύο εκατοστά πιο
 * κάτω, από το ΙΔΙΟ νούμερο: «Αρκετός αέρας αλλά θάλασσα λάδι». Και η κάρτα της λίστας, από
 * όπου ήρθε ο επισκέπτης, «Αρκετός αέρας». Το «4–5 Μπφ» είναι ο ωμός άνεμος του κελιού, το
 * «απάνεμη» η γεωμετρία της ακτής — αλλά τίποτα στην οθόνη δεν έλεγε ποιο είναι ποιο, οπότε
 * ο ίδιος άνθρωπος διάβαζε «φυσάει» και «δεν φυσάει» για την ίδια παραλία.
 *
 * Η ΛΥΣΗ ΕΙΝΑΙ ΜΙΑ ΚΛΙΜΑΚΑ, ΟΧΙ ΔΥΟ ΛΕΞΙΛΟΓΙΑ. Από 4 Μπφ και πάνω η λέξη του πλακιδίου είναι
 * η ΙΔΙΑ κρίση με τη λέξη της κάρτας (utils/conditionsFeelPhrase.windFeelLevel), απλώς σε
 * ρήμα γιατί κάθεται κάτω από αριθμό:
 *
 *     κάρτα «Αρκετός αέρας» (4)  → πλακίδιο «φυσάει αρκετά»
 *     κάρτα «Πολύς αέρας»   (5)  → πλακίδιο «φυσάει πολύ»
 *     κάρτα «Δυνατός αέρας» (6+) → πλακίδιο «φυσάει δυνατά»   ← υπήρχε ήδη από 14/08
 *
 * Το τελευταίο σκαλί το είχε ήδη πάρει η βίβλος μόνη της· τα δύο από κάτω έλειπαν, και σε
 * αυτό ακριβώς το κενό ζούσε η αντίφαση. Δύο συνώνυμα για το ίδιο πράγμα σε μία οθόνη
 * διαβάζονται ως δύο διαφορετικά πράγματα — ίδιος κανόνας με τη λεζάντα του χάρτη.
 *
 * ΚΑΙ ΤΟ «ΚΑΤΑΜΟΥΤΡΑ» ΜΠΗΚΕ ΣΤΗΝ ΙΔΙΑ ΚΛΙΜΑΚΑ (ίδια μέρα, δεύτερη ανάγνωση του Μίλτου: «δεν
 * μου αρέσει που το λέει ορισμένες φορές το πλακίδιο»). Ήταν η τελευταία λέξη που μιλούσε
 * ΓΩΝΙΑ σε ένα πλακίδιο όπου όλες οι άλλες μιλούν πια ΕΝΤΑΣΗ, και δίπλα σε 5-6 Μπφ ακουγόταν
 * σαν προειδοποίηση τρίτης κλίμακας — μετά το νούμερο και μετά το χρώμα. Πάνω από το κατώφλι
 * της υπόσχεσης καμία ακτή δεν λέει γεωμετρία: και οι τρεις εκθέσεις λένε πόσο φυσάει.
 *
 * ΤΙ ΚΟΣΤΙΣΕ, ΡΗΤΑ: από 4 Μπφ και πάνω το πλακίδιο του ανέμου ΔΕΝ ξεχωρίζει πια υπήνεμη,
 * πλάγια και κατάμουτρα ακτή — λέει ένταση, όπως η κάρτα. Η έκθεση δεν χάνεται από τη σελίδα:
 * τη λένε το διπλανό πλακίδιο («~0,1 μ.» δίπλα σε «0,3 μ. ανοιχτά»), το χρώμα του κελύφους,
 * το τσιπάκι «Πιο προστατευμένη» της κάρτας και η στατική ενότητα του τοπικού ανέμου. Αυτό
 * που χάνεται είναι μια ΥΠΟΣΧΕΣΗ νηνεμίας που δεν την υπολογίζουμε (μετράμε τη σκιά στο ΚΥΜΑ,
 * όχι πόσα μποφόρ λιγότερα πέφτουν στην άμμο) και μια γωνία που ο επισκέπτης δεν ζήτησε.
 *
 * ΜΟΝΟΔΡΟΜΗ, ΟΠΩΣ ΟΛΕΣ: η κλίμακα μπορεί μόνο να αφαιρέσει ηρεμία. Οι λέξεις της γεωμετρίας
 * —«απάνεμη», «αεράκι», «κατάμουτρα»— επιβιώνουν ΜΟΝΟ στα 3 Μπφ
 * (SHELTER_WORD_CALM_PROMISE_MAX_BEAUFORT), όπου η κάρτα λέει «Λίγος αέρας», καμία τους δεν
 * διαφωνεί μαζί της, και η γωνία είναι το μόνο πράγμα που ο αριθμός δεν λέει. Αριθμός, χρώμα,
 * ετυμηγορία, κατάταξη: αμετάβλητα — αλλάζει μία λέξη σε ένα πλακίδιο.
 *
 * ΤΙ ΑΝΤΙΚΑΤΕΣΤΗΣΕ. Η πύλη στεριάς της 27/08 (Γλυφάδα #1993: 'protected' για το κύμα, αλλά ο
 * βοριάς έτρεχε κατά μήκος της ακτής) γύριζε την «απάνεμη» σε «φυσάει» μόνο όταν το windShadow
 * έλεγε ότι ο άνεμος ΔΕΝ ήρθε από στεριά. Ο νέος κανόνας την καταπίνει: στα 4+ καμία υπήνεμη
...
/**
 * ΩΣ ΕΔΩ ΤΟ ΠΛΑΚΙΔΙΟ ΕΠΙΤΡΕΠΕΤΑΙ ΝΑ ΥΠΟΣΧΕΘΕΙ ΗΡΕΜΙΑ. Από 4 Μπφ και πάνω, «απάνεμη» και
 * «αεράκι» δεν λέγονται — μπαίνει η κλίμακα της κάρτας σε ρήμα («φυσάει αρκετά» / «φυσάει
 * πολύ» / «φυσάει δυνατά»). Δες το μπλοκ του SHELTER_LABEL για το γιατί.
 *
 * ΓΙΑΤΙ ΤΟ 3 ΚΑΙ ΟΧΙ ΤΟ 2: στα 3 Μπφ η κάρτα λέει «Λίγος αέρας» και η «απάνεμη» δεν διαφωνεί
 * μαζί της — ίδιο κατώφλι με το WIND_NOTE_MIN_BEAUFORT της γραμμής ανέμου, και το ίδιο σημείο
 * όπου το BeachDetailPage αρχίζει να τυπώνει λέξη καθόλου (κάτω από 3 η «απάνεμη» θα ήταν
 * διαπίστωση για άνεμο που δεν φυσάει).
 *
 * ⚠️ 11/09/2026 — Η «ΑΠΑΝΕΜΗ» ΔΕΝ ΤΥΠΩΝΕΤΑΙ ΠΙΑ ΟΥΤΕ ΣΤΑ 3 ΜΠΦ (Πυργάκι Νάξου #2013, απόφαση Μίλτου·
 * η εξαίρεση ζει στο BeachDetailPage, δίπλα στον υπολογισμό της λέξης). Η προστατευμένη ακτή λέει
 * ό,τι η κάρτα («φυσάει λίγο»). Το κατώφλι αυτό κρατά πλέον μόνο το «αεράκι» και το «κατάμουτρα»
 * στα 3 — το `protected` του SHELTER_LABEL μένει γιατί το ζητά η πύλη διακριτών λέξεων.
 *
 * ΤΙ ΕΓΙΝΕ Η ΠΥΛΗ ΣΤΕΡΙΑΣ ΤΗΣ 27/08 (SHELTER_WORD_LAND_GATE_MIN_BEAUFORT = 4, καταργήθηκε
 * 28/08). Είχε μπει για τη Γλυφάδα Νάξου #1993: 'protected' για το ΚΥΜΑ (άνοιγμα 0,96 χλμ),
 * αλλά το `windShadow` έλεγε ότι ο βοριάς ΔΕΝ πέρασε πάνω από στεριά — έτρεχε κατά μήκος της
 * ακτής και χτυπούσε τον επισκέπτη αφρέναρος. Γύριζε τότε τη λέξη σε «φυσάει» μόνο σε αυτή την
 * περίπτωση. Ο κανόνας των 4 Μπφ την καταπίνει ολόκληρη: πλέον ΚΑΜΙΑ υπήνεμη ακτή δεν υπόσχεται
 * νηνεμία σε αυτή τη ζώνη, με σκιά ή χωρίς, οπότε η Γλυφάδα διορθώνεται χωρίς δεύτερο
 * μηχανισμό — και οι μάρτυρες που εκείνη η πύλη προστάτευε (Φυριπλάκα #1927, Βάι #730,
 * Λιβάδια #2033) απλώς διαβάζουν τώρα την ένταση αντί για την υπόσχεση. Η μέτρησή της μένει
 * στο reports/weather/shelter-word-land-gate-2026-08-27.json.
 */
export const SHELTER_WORD_CALM_PROMISE_MAX_BEAUFORT = 3;
