...
export const BeachSearcherHome: React.FC<BeachSearcherHomeProps> = ({
  ...
  // When the user picks a beach from search, App scrolls the page to the map and fires
  // this nonce-keyed signal so we horizontally centre that beach's card in whichever
  // carousel holds it — landing them on "map on top, the beach's card right below".
  // Horizontal-only (scrollBy on the carousel), so it never fights the vertical page
  // scroll to the map. The card-nearest-centre sync effect then keeps the pin in step.
  useEffect(() => {
    ...
    // Double rAF: let the name-search-filtered carousels re-render (and, on a
    // cross-region jump, the new island mount) before we measure and centre.
