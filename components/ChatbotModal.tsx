...
import { aiOptions } from '../services/geminiService';

interface ChatbotModalProps {
    isOpen: boolean; onClose: () => void; t: Translation; messages: any[];
    onSend: (message: string, model: string) => void; isLoading: boolean;
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-[90]">
            <div className="bg-white dark:bg-slate-900 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg h-[95vh] sm:h-[80vh] flex flex-col shadow-2xl overflow-hidden">
                <header className="p-4 border-b dark:border-slate-800 flex justify-between items-center">
                    <div>
                        <h2 className="font-bold dark:text-white">Beach Buddy Chat</h2>
                        <select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} className="text-xs bg-slate-100 dark:bg-slate-800 dark:text-slate-400 p-1 rounded">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    {isLoading && <div className="text-slate-400 italic text-sm animate-pulse">Η 3090 σκέφτεται...</div>}
                </div>
                <form className="p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] border-t dark:border-slate-800 flex gap-2" onSubmit={(e) => { e.preventDefault(); if(input.trim()) { onSend(input, selectedModel); setInput(''); } }}>
                    <input value={input} onChange={e => setInput(e.target.value)} className="flex-1 bg-slate-100 dark:bg-slate-800 dark:text-white rounded-full px-4 py-3 outline-none text-sm" placeholder="Ρώτησε κάτι..." />
