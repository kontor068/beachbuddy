...
const FilterModal: React.FC<FilterModalProps> = ({ isOpen, onClose, t, children, resultCount }) => {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[95dvh] w-full max-w-2xl transform flex-col overflow-hidden rounded-t-surface bg-canvas shadow-lifted transition-transform duration-300 animate-slide-up sm:max-h-[85vh] sm:rounded-surface"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex flex-shrink-0 items-center justify-between gap-3 border-b border-line bg-canvas px-4 py-3.5 backdrop-blur sm:px-5">
          <h2 id="filter-modal-title" className="flex min-w-0 items-center gap-2 text-xl font-bold text-slate-800">
            ...
            {typeof resultCount === 'number' && (
              <span className="shrink-0 rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-extrabold text-cyan-800 ring-1 ring-cyan-100">
                {resultCount}
              </span>
            )}
          </h2>
