...
*Snapshot date: 28 July 2026, re-checked 30 August 2026 — every number below was counted in the
repository. Items marked (30/08) were corrected on the re-check.*
...
- **2.872 beaches shipped** (2.926 in the canonical file) across **110 regions/islands** (30/08)
...
- **~9.500 pre-rendered pages** (9.510 built, 9.829 in the sitemap across five locales): beach
  pages, region pages, **381 island-intent guides**, 13 national landings, a guides hub, a
  FAQ, the methodology article, legal pages (30/08 — the "963 guides" figure was wrong)
- URL shapes: `/beaches/{region}/{id}-{slug}/`, `/el/family-beaches/{region}/`,
  `/el/organized-beaches/{region}/`, `/el/beaches/{island}/`
- Photos on **1.367 beaches (47,6%)** — `reports/photo-coverage/beach-photo-presence-summary.txt`,
  17/08 — plus 27 that exist but are withheld for a missing licence credit. Curated bilingual
  editorial text on **788** (30/08)
...
No state-management library — `App.tsx` holds it. No database and no server: **14 Netlify
functions** (30/08) — forecast proxy, cookieless pageview counter, traffic stats, feedback→Telegram,
client-error/CSP sink, ensemble spread, weekly quality digest, health, photo moderation + notify,
newsletter, account deletion, Telegram webhook, feedback export.
...
Quality is enforced by **85 automated checks** (`npm run quality:critical`) running in CI on
every pull request — roughly 55 of them on weather/sea truth and copy consistency, plus a
Playwright layout probe and, since 30/08, a bundle-weight ceiling. A second workflow
re-validates ~2.900 beach-hours against real wind twice a week. (30/08 — "12" was long stale.)
...
| "There is no way to report a problem" | Four live feedback paths, delivering to Telegram (30/08) |
| "There is no Search Console" | Verified property, service-account API pull, snapshots in `reports/snapshots/` |
| "Core Web Vitals are not measured" | `utils/webVitals.ts` since 30/08 — LCP/INP/CLS from real visitors |

## Real gaps as of 30 August 2026

Every gap the 28 July version of this section listed has since been closed — photos and credits
now render in the prerendered HTML, the static footer carries the safety disclaimer and the
legal links on every page type, the ODbL attribution is stated, and error/CSP reporting plus a
health endpoint exist. What is actually open now:

**1.505 beaches have no photo** (52,4%) and six regions have none at all; `npm run
quality:photo-coverage` ranks the missing ones by importance. **The DE/FR/IT pages earn
impressions and almost no clicks** — /it 0,4% CTR on 2.925 impressions against 3,9% for en/el,
and all three are served mostly inside Greece (`reports/snapshots/2026-08-21.md`). **Netlify
function invocations have no ceiling and no alarm** — the weather quota and build credits both
do. **Nothing a visitor reports is ever shown back to another visitor**, so the site has no
third-party proof of any kind. And **the founder's own hours are still not counted**, which is
the constraint that decides the order of everything above.
