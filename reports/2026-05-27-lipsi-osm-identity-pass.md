# Lipsi OSM identity/location pass - 2026-05-27

## Scope

Continued the Phase 2 second-pass data work with a focused OSM identity/location pass for the Lipsi bucket in the South Aegean / Dodecanese dataset.

## What changed

Added direct OpenStreetMap source URLs and conservative source notes for 15 low-confidence Lipsi records that had exact, near-exact, transliterated, or otherwise compatible OSM identity/location matches.
...
- Lipsi beaches: 18
- Records with direct source URLs: 16
- Low-confidence records without source URLs: 0

Lipsi now has no low-confidence app-facing records without direct source URLs.

## Validation

- `npm run build:beach-data`: passed, 2720 beaches split into 110 region files
- `node scripts/auditBeachVerificationCoverage.mjs`: passed, 1823 / 2720 app-facing records now have direct source URLs
