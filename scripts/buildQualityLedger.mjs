...
for (const region of index.regions) {
  ...
  // ── ΤΑ ΚΕΝΑ, ΜΕ ΤΟΝ ΑΞΟΝΑ ΤΟΥΣ ΔΙΠΛΑ ──────────────────────────────────────────
  // Κάθε γραμμή κουβαλάει τον άξονα και το πλήθος της, όχι μόνο το κείμενό της.
  // Το ταμπλό τις δείχνει όλες, οπότε δεν το ένοιαζε· το εβδομαδιαίο μήνυμα όμως
  // κρατάει δύο, και όσο οι γραμμές ήταν σκέτα κείμενα σε σταθερή σειρά αξόνων,
  // κρατούσε τις ΠΡΩΤΕΣ ΔΥΟ — όχι αυτές που μόλις είχε ονομάσει από πάνω.
  //
  // Μετρημένο 31/08/2026 πάνω στο ημερολόγιο της 25/08: η Κύθνος έγραφε επικεφαλίδα
  // «Παροχές 26%» και από κάτω «1 χωρίς επιβεβαιωμένες οδηγίες · 3 με άγνωστη
  // πρόσβαση» — τέσσερις παραλίες, ενώ οι 29 χωρίς καμία παροχή, ο σωρός που η ίδια
  // η επικεφαλίδα μόλις είχε δείξει, δεν φαινόταν πουθενά. Ένα μήνυμα που αναιρεί
  // τον εαυτό του δύο γραμμές πιο κάτω δεν το ξαναδιαβάζει κανείς.
  //
  // `sub: true` σημαίνει «υπο-γραμμή»: εξηγεί την από πάνω της, δεν στέκεται μόνη.
  const gaps = [];
  const gapLine = (key, n, one, many, sub = false) => {
    if (n > 0) gaps.push({ key, count: n, text: n === 1 ? one : many.replace('{n}', String(n)), ...(sub ? { sub: true } : {}) });
  };
  gapLine('pin', openNames.pin.length, '1 παραλία με πινέζα εκτός παραλίας', '{n} παραλίες με πινέζα εκτός παραλίας');
  gapLine('nav', openNames.nav.length, '1 χωρίς επιβεβαιωμένες οδηγίες', '{n} χωρίς επιβεβαιωμένες οδηγίες');
  gapLine(
    'nav',
  ...
  gapLine('access', openNames.access.length, '1 με άγνωστη πρόσβαση', '{n} με άγνωστη πρόσβαση');
  gapLine('amenities', openNames.amenities.length, '1 χωρίς καμία παροχή γραμμένη', '{n} χωρίς καμία παροχή γραμμένη');
  gapLine('photo', openNames.photo.length, '1 χωρίς φωτογραφία', '{n} χωρίς φωτογραφία');
  gapLine(
    'character',
  ...
  gapLine('text', openNames.text.length, '1 με χαμηλή σιγουριά κειμένου', '{n} με χαμηλή σιγουριά κειμένου');
  gapLine('text', needsVerification, '1 σημειωμένη «θέλει επαλήθευση»', '{n} σημειωμένες «θέλουν επαλήθευση»', true);
