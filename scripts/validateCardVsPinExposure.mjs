...
const KNOWN = new Map(Object.entries({
  // Το χειρόγραφο προφίλ επιστρέφεται στη γραμμή 346 του mapExposure ΠΡΙΝ ελεγχθεί η γεωμετρία
  // 'exposed' στη γραμμή 390. Εδώ η γεωμετρία λέει ένταση 83,5 / fetch 10 χλμ / onshore 0,99 σε
  // βορειοδυτικό — δηλαδή μελτέμι κατάμουτρα — και η πινέζα βγαίνει ΠΡΑΣΙΝΗ.
  '2020@NW': 'Αγία Ειρήνη Πάρου — authored override νικάει γεωμετρία exposed (i=83,5)',
  ...
  '1726@SE': 'Χρυσή Άμμος Άνδρου — suspectPin σβήνει γεωμετρία exposed',
  '1726@S': 'Χρυσή Άμμος Άνδρου — suspectPin σβήνει γεωμετρία exposed',
  '2049@SE': 'Πούντα Πάρου — suspectPin σβήνει γεωμετρία exposed',
  '2049@S': 'Πούντα Πάρου — suspectPin σβήνει γεωμετρία exposed',
...
/**
 * ΟΙ ΠΑΡΑΛΙΕΣ ΟΠΟΥ ΤΟ ΠΕΡΑΣΜΑ ΓΕΙΤΟΝΙΑΣ ΑΛΛΑΖΕΙ ΤΟ ΧΡΩΜΑ — μετρημένες 20/08/2026.
 *
 * Ο εξομαλυντής γειτόνων παραβιάζει τον κανόνα σε 388 τομείς, αλλά στους 299 από αυτούς το
 * χρώμα της πινέζας ΔΕΝ αλλάζει (η σκάλα χρώματος δεν ξεχωρίζει τις δύο ζώνες σε εκείνη την
 * ένταση). Ο χρήστης βλέπει τη διαφορά μόνο στις 89 από κάτω, σε 11 παραλίες — και αυτές
 * σταματούν το build. Οι υπόλοιπες μετριούνται και τυπώνονται ώστε να μη μεγαλώνουν στα κρυφά.
 *
 * ΔΙΑΒΑΣΕ ΠΟΙΕΣ ΕΙΝΑΙ ΠΡΙΝ ΤΙΣ ΘΕΩΡΗΣΕΙΣ ΘΟΡΥΒΟ: Άγιος Ιωάννης Λευκάδας, Φτελιά Μυκόνου,
 * Μικρή Βίγλα Νάξου. Τρία από τα γνωστότερα σημεία kitesurf της χώρας βάφονται πιο ΗΡΕΜΑ απ'
 * ό,τι λέει η ίδια τους η κάρτα, επειδή ένας curated γείτονας τους δανείζει προστασία μέσω
 * `hasCuratedSegmentProtectionSupport`. Η διαρροή σε γείτονες ήταν γνωστός κίνδυνος από
 * 17/08/2026 (§Γ15) — δεν είχε μετρηθεί ποτέ ότι φτάνει ως το χρώμα.
 */
const KNOWN_COLOUR = new Map(Object.entries({
  1146: 'Άγιος Ιωάννης Λευκάδας — kitesurf spot· κίτρινο→μπλε στα 3 Μπφ',
  1976: 'Φτελιά Μυκόνου — kitesurf spot· πορτοκαλί→κίτρινο στα 4 Μπφ',
  2006: 'Μικρή Βίγλα Νάξου — kitesurf spot· πορτοκαλί→κίτρινο στα 4 Μπφ',
  2158: 'Αποθήκες Τήνου — πορτοκαλί→κίτρινο στα 4 Μπφ',
  2166: 'Κολυμπήθρα Τήνου — πορτοκαλί→κίτρινο στα 4 Μπφ',
  1811: 'Καλησκιά Κέας — κόκκινο→πορτοκαλί στα 5 Μπφ',
  1963: 'Μερχιά Μυκόνου — κόκκινο→πορτοκαλί στα 5 Μπφ',
  2001: 'Λιγαρίδια Νάξου — κόκκινο→πορτοκαλί στα 5 Μπφ',
  2079: 'Γιαλούδι Σερίφου — κόκκινο→πορτοκαλί στα 5 Μπφ',
  2081: 'Καλόγερος Σερίφου — κόκκινο→πορτοκαλί στα 5 Μπφ',
  2111: 'Τσόχα Σίφνου — κόκκινο→πορτοκαλί στα 5 Μπφ',
}));
