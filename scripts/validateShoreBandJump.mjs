 ...
 *
 * ⚠️ ΜΙΑ ΜΟΝΗ ΕΞΑΙΡΕΣΗ, ΠΡΟΣΤΕΘΗΚΕ 25/08/2026: ΤΟ ΔΑΠΕΔΟ ΤΩΝ 0,10 μ.
 *
 * Αναφορά χρήστη από τη Λυγιά Λευκάδας (#1165, 09:00): η κάρτα έγραφε «0,0 μ.». Ο άνεμος ΗΤΑΝ
 * άπνοια (2-4 χλμ/ώρα σε τέσσερα ανεξάρτητα μοντέλα) και καμία θάλασσα δεν έμπαινε — δηλαδή ο
 * αριθμός ακτής ήταν σωστός. Λάθος ήταν το ΤΥΠΩΜΕΝΟ μηδέν: «0,0 μ.» διαβάζεται ως χαλασμένη
 * ένδειξη, όχι ως ήρεμο νερό, και αρκεί ένα απόνερο βάρκας για να το διαψεύσει. Ο κώδικας το
 * έγραφε ήδη σε τρία σημεία (`WAVE_DISPLAY_FLOOR_M`, `SHORE_DISPLAY_FLOOR_M`,
 * `COVE_DISPLAY_FLOOR_M`) χωρίς να το εφαρμόζει σε αυτή τη διαδρομή. Μετρήθηκε 25/08 σε 5
 * περιοχές: 406 από 1.441 ώρες-παραλίας (28,2%) τύπωναν σκέτο «0,0 μ.».
 *
 * ΓΙΑΤΙ ΑΥΤΟ ΔΕΝ ΕΙΝΑΙ Ο ΦΡΑΧΤΗΣ ΞΑΝΑ, ΚΑΙ Η ΠΥΛΗ ΤΟ ΑΠΟΔΕΙΚΝΥΕΙ. Ο φράχτης σήκωνε τον αριθμό
 * ως τη ΓΡΑΜΜΗ AMBER — μεταβλητό ύψος, ως 0,8 μ. στο Βάι — για να συμφωνήσει με το χρώμα. Το
 * δάπεδο σηκώνει μόνο ως το 0,10 μ., σταθερά, και μόνο από κάτω του. Ο έλεγχος (1α) παρακάτω
 * απαιτεί ρητά ότι ο τυπωμένος αριθμός είναι Η ΑΚΤΗ Ή ΑΚΡΙΒΩΣ ΤΟ ΔΑΠΕΔΟ — τίποτα ενδιάμεσο,
 * τίποτα ψηλότερα. Ένας φράχτης δεν μπορεί να ξαναπεράσει από εδώ μεταμφιεσμένος σε δάπεδο.
 *
 * ΤΙ ΦΥΛΑΕΙ ΤΩΡΑ:
 *   (1) το τυπωμένο νούμερο είναι ΑΚΡΙΒΩΣ ο αριθμός ακτής (με ταβάνι το ανοιχτό νερό) —
 *       κανένας «τρίτος» κατασκευασμένος αριθμός ανάμεσά τους· μόνη εξαίρεση το δάπεδο·
 *   (1α) όπου το δάπεδο μιλάει, τυπώνεται ΑΚΡΙΒΩΣ 0,10 μ. — ποτέ ένα ενδιάμεσο ύψος·
 ...
 *   (4) τέσσερις ονομαστικές δεσμεύσεις: Βάι τυπώνει 0,1 πάνω από θάλασσα 1,14 (η περίπτωση που
 *       γέννησε την απόφαση)· ο ήρεμος όρμος 0,3→0,1 μένει 0,1· η Παραλία Μαραθώνα τυπώνει
 *       το δικό της 0,69 — ΟΧΙ το 0,8 του παλιού φράχτη, ΟΧΙ το 1,38 του ανοιχτού· και η Λυγιά
 *       Λευκάδας (ανοιχτά 0,02, ακτή 0,01) τυπώνει «0,1 μ.» ΧΩΡΙΣ «~» — δύο αριθμοί κάτω από
 *       το δάπεδο είναι ο ΙΔΙΟΣ τυπωμένος αριθμός, όχι μια εκτίμηση δίπλα σε μια μέτρηση.
 *
...
// Εισάγεται, ΔΕΝ ξαναγράφεται: αν το δάπεδο αλλάξει ποτέ τιμή, η πύλη πρέπει να κινηθεί μαζί του
// αντί να αρχίσει να κρίνει έναν αριθμό που κανείς δεν τυπώνει πια.
const { WAVE_DISPLAY_FLOOR_M } = require(path.join(root, 'utils/waveModel.ts'));
...
for (const openM of OPEN_WATER_M) {
  for (const periodS of PERIODS_S) {
    for (const factor of SHORE_FACTORS) {
      ...
      // (1) Ο ΑΡΙΘΜΟΣ ΑΚΤΗΣ, ΑΥΤΟΥΣΙΟΣ. Κανένα ανασήκωμα, κανένα τρίτο νούμερο — με τη μόνη
      // εξαίρεση το δάπεδο εμφάνισης των 0,10 μ. (βλ. κεφαλίδα, 25/08/2026).
      const shoreCapped = Math.min(shoreM, openM);
      const expected = Math.max(shoreCapped, WAVE_DISPLAY_FLOOR_M);
      if (typeof shown !== 'number' || Math.abs(shown - expected) > 1e-9) {
        failures.push(`ΑΛΛΟΙΩΜΕΝΟ ΝΟΥΜΕΡΟ: ${label} — έπρεπε ${expected}μ`);
        continue;
      }

      // (1α) ΤΟ ΔΙΧΤΥ ΑΠΕΝΑΝΤΙ ΣΕ ΝΕΟ ΦΡΑΧΤΗ. Όπου ο αριθμός ΔΕΝ είναι η ακτή, επιτρέπεται να
      // είναι ΜΟΝΟ το δάπεδο — ποτέ κάποιο ενδιάμεσο ύψος προς τη γραμμή ηρεμίας.
      const isShoreItself = Math.abs(shown - shoreCapped) <= 1e-9;
      const isFloorItself = Math.abs(shown - WAVE_DISPLAY_FLOOR_M) <= 1e-9;
      if (!isShoreItself && !isFloorItself) {
        failures.push(`ΤΡΙΤΟ ΝΟΥΜΕΡΟ: ${label} — ούτε η ακτή (${shoreCapped}μ) ούτε το δάπεδο (${WAVE_DISPLAY_FLOOR_M}μ)`);
      }

      // (2) ΠΟΤΕ ΠΑΝΩ ΑΠΟ ΤΟ ΑΝΟΙΧΤΟ ΝΕΡΟ — που κι αυτό τυπώνεται με το ίδιο δάπεδο, ώστε τα
      // δύο νούμερα της σελίδας παραλίας να μη γεννήσουν ψεύτικη διαφορά κάτω από τα 0,10.
      const openPrintedM = Math.max(openM, WAVE_DISPLAY_FLOOR_M);
      if (shown > openPrintedM + 1e-9) {
      ...
      const differs = Math.abs(shown - openPrintedM) >= 0.05;
...
// Λυγιά Λευκάδας #1165, 25/08/2026, 09:00 — η αναφορά που γέννησε το δάπεδο. Ανοιχτά 0,02 μ.,
// ακτή 0,01 μ. (K_d 0,1 πάνω σε ήδη μηδενικό νερό): τυπώνεται 0,1 και ΧΩΡΙΣ «~», γιατί μετά το
// δάπεδο οι δύο αναγνώσεις ταυτίζονται — δεν υπάρχει εκτίμηση να ξεχωρίσει από μέτρηση.
const lygia = buildBeachConditionsReadout({
  beachWindSpeedKmph: 5, waveHeightM: 0.02, seaStateWaveM: 0.02, seaStatePeriodS: 1.95,
  shoreDisplayWaveM: 0.01, language: 'gr',
});
if (lygia.waveM !== 0.1 || lygia.waveText !== '0,1 μ.') {
  failures.push(`ΛΥΓΙΑ: τυπώθηκε ${lygia.waveM}μ («${lygia.waveText}») αντί 0,1 («0,1 μ.», χωρίς «~»)`);
}
...
console.log('Βάι 0,1/1,14 ✓ · ήρεμος όρμος 0,1/0,3 ✓ · Μαραθώνας 0,69/1,38 ✓ · Λυγιά 0,1/0,02 ✓ — κανένα κατασκευασμένο νούμερο.');
