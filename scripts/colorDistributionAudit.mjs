...
const outDir = path.join(root, '.tmp', 'color-distribution-audit');
...
try {
  execFileSync(process.execPath, [tsc,
    ...
    '--outDir', outDir,
    '--noEmit', 'false',
    path.join(root, 'scripts', 'colorDistributionAudit.ts'),
  ], {
    cwd: root,
    stdio: 'inherit',
  });
  ...
  execFileSync(process.execPath, [
    path.join(outDir, 'scripts', 'colorDistributionAudit.js'),
