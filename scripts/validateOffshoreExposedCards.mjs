 ...
 * 118 τομείς σε 59 παραλίες, από δύο ξεχωριστές αιτίες. Και οι δύο κυνηγήθηκαν ως το τέλος:
 *
 *   • 67 τομείς / 23 παραλίες από τη σημαία `knownWindSportSpot`, που γύριζε 'exposed' χωρίς να
 *     κοιτάξει κατεύθυνση. ΔΙΟΡΘΩΘΗΚΕ (§Γ28β) — η σημαία έγινε κατευθυντική. 118 → 51.
 *   • 51 τομείς / 40 παραλίες από χειρόγραφο `exposedToWindDirections` πάνω σε λανθασμένο
 *     `beachFacingDirection`. Έμεινε ανοιχτό όσο κανένας μάρτυρας του repo δεν ήταν ανεξάρτητος
 *     (§Γ28δ). Έκλεισε με την ακτογραμμή του OpenStreetMap (§Γ28ε), που έκρινε 38/40 υπέρ της
 *     γεωμετρίας· 24 εγγραφές διορθώθηκαν και 10 ΕΠΑΝΑΦΕΡΘΗΚΑΝ (§Γ28στ). 51 → 22.
 *
 * ΓΙΑΤΙ ΜΕΝΟΥΝ 22. Δεκατρείς παραλίες δεν πήραν καθαρή ετυμηγορία από τον OSM (πινέζα μακριά
 * από την ακτογραμμή, ή μεγάλη διασπορά επειδή είναι μύτη με θάλασσα και στις δύο πλευρές —
 * #2457 Πρασονήσι είναι το καθαρό παράδειγμα). Και σε λίγες, το 'exposed' δεν έβγαινε ποτέ από
 * τη λίστα αλλά από τη ΓΩΝΙΑΚΗ εφεδρεία πάνω στο ίδιο λανθασμένο facing — που ΔΕΝ διορθώθηκε
 * επίτηδες, γιατί κουνάει τομείς που δεν μετρήθηκαν. Είναι ουρά επαλήθευσης εδάφους, όχι bug.
 *
 * Ο αριθμός επιτρέπεται ΜΟΝΟ ΝΑ ΠΕΦΤΕΙ. Αύξηση σημαίνει ότι κάποιος ξανάκανε ακατεύθυντο έναν
 * κανόνα έκθεσης, ή ότι μπήκαν χειρόγραφοι τομείς πάνω σε γεωμετρία που τους διαψεύδει.
 *
 * ΤΙ ΚΛΕΙΔΩΝΕΙ:
 *   Α. Το πλήθος δεν ξεπερνά τη βάση (22 τομείς / 17 παραλίες).
...
/** Μετρημένη βάση 20/08/2026 μετά τις διορθώσεις §Γ28β και §Γ28στ. Επιτρέπεται μόνο να πέσει. */
const BASELINE_SECTORS = 22;
const BASELINE_BEACHES = 17;
  ...
  for (const beach of beaches) {
    ...
    for (const scen of SCEN) {
      ...
      for (const { bft, kmh } of BFS) {
        ...
        // ΑΠΟΔΟΣΗ ΑΙΤΙΑΣ, ΟΧΙ ΕΙΚΑΣΙΑ. Η πρώτη εκδοχή έλεγε «σημαία windsurf» για κάθε τομέα
        // που ΔΕΝ ήταν στη χειρόγραφη λίστα — και κατηγόρησε τη σημαία για την Πούντα Πάρου
        // (20/08/2026), όπου το 'exposed' έβγαινε στην πραγματικότητα από τη γωνιακή εφεδρεία
        // πάνω σε λανθασμένο authored facing. Η σημαία σηκώνεται μόνο από 4 Μπφ και πάνω, άρα
        // αν ο τομέας βγαίνει «Εκτεθειμένη» ΚΑΙ στα 3, η αιτία είναι αλλού εξ ορισμού.
        const exposedAtThree = bft === 3 || Boolean(existing?.exposedAtThree);
        const fromWindSportFlag = Boolean(profile.knownWindSportSpot) && bft >= 4 && !exposedAtThree
          && !(profile.exposedToWindDirections || []).includes(assessment.windSector);
        if (existing) {
          existing.exposedAtThree = existing.exposedAtThree || bft === 3;
          existing.windSport = existing.windSport || fromWindSportFlag;
          continue;
        }
        hits.set(key, {
