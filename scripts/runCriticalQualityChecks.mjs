...
const checks = [
  ...
  },
  {
    ...
    description: 'Drives the REAL weighted table: the weights sum to 100 and split 70 weather / 30 comfort, and the region podium is proved blind to the visitor\'s location — a beach 2 km away and one 240 km away, identical in everything else, rank in the order they arrived; an exposed beach with perfect access, full amenities and the best crowd tier can never outscore a protected one on identical wind and sea; two sea heights inside the model\'s error bar score identically and SEA_STEP_M still equals PODIUM_SEA_MEANINGFUL_DIFFERENCE_M; a beach with no wave, no exposure verdict and no crowd tier scores in the middle rather than at the floor; no crowd tier ever adds points or removes more than 5; and across every region holding a paid beach, at every wind state, none reaches a Top 3. Self-proves with --prove: comfort raised to 50, the sea step made continuous, missing data scored zero, and the crowd penalty flipped to a bonus must each make it fail.',
    ...
    failureAction: 'Fix utils/topPickScoreTable.ts, or — for the location assertion — services/topPickRanking.ts, where `distanceKm: undefined` is deliberate and must not be "restored" to read item.distance. Never rebalance the weights to make a case pass — the 70/30 split is a product decision recorded in the status board, and moving it silently re-weights every recommendation on the site. Never make an axis continuous to "break more ties": a tie means the evidence genuinely cannot separate two beaches.',
    command: process.execPath,
