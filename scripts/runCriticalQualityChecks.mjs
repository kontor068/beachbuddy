...
const checks = [
  ...
  },
  {
    ...
    protects: 'Το σφάλμα είναι αόρατο σε ανάγνωση — η εγγραφή φαίνεται σωστή, απλώς ο κώδικας κοιτάει αλλού. Μετρημένο 25/08/2026: εννιά παραλίες σημαδεμένες για απόκρυψη σερβίρονταν κανονικά για μήνες, ανάμεσά τους ένα διπλότυπο άλλης κάρτας και μία που οι πηγές τη βάζουν σε άλλο νησί.',
    failureAction: 'Τρέξε `node scripts/fixRootLevelExcludeFlag.mjs --write`. Αν μια σημαία δεν έχει γραμμένο λόγο, γράψε ΠΡΩΤΑ τον λόγο — απόκρυψη σελίδας χωρίς αιτιολογία δεν είναι απόφαση, είναι υπόλοιπο.',
    command: process.execPath,
    args: ['scripts/validateExcludeFlagPlacement.mjs'],
  },
  {
  ...
  {
    id: 'shore-band-jump',
    title: 'The printed metre figure is always the water at the shore, unaltered',
    description: 'Drives utils/beachConditionsReadout over a 300-case grid of open-water heights x wave periods x shore discounts, and checks the number the card and map print is EXACTLY the shore figure (capped by the open-water reading, never above it), that no third fabricated number appears between the two, that the «~» estimate marker appears when and only when shore and open water differ by ≥0,05 m, and three named commitments: Βάι prints its 0,1 m over a 1,14 m sea (the case that set the rule), a genuine calm cove at 0,3 m stays 0,1, and Παραλία Μαραθώνα prints its own 0,69 — not the old fence value 0,8 and not the open 1,38.',
    protects: 'The number tells the truth about the shore; the warning lives in the pin colour and the swim verdict, which still judge on decision-grade sea state (Miltos, 24/08/2026 — Vai). HISTORY, so nobody rebuilds the old rule as a fresh finding: from 21/08 to 24/08 this gate enforced the OPPOSITE — a «calm-line fence» that raised any shore figure falling into the calm band while the open sea was above it (measured nationally: 105 and 178 beaches on two meltemi days, 100% under a non-calm pin, §Γ47/§Γ49, exception §Γ55/§Γ56). Its symptom killed it: at Vai the card printed a fabricated «~0,8 μ.» over 0,1 m water at the sand, beside a beach page saying 0,1 — scaring visitors away from the flattest water in Crete on a meltemi day. Known cost of the reversal, accepted deliberately: on meltemi days the x0,5-discount beaches print a calm-looking number under a warning pin; the x0,5 itself stays uncalibrated (PORISMA §7δ).',
    failureAction: 'Fix utils/beachConditionsReadout.ts so the printed figure is min(shore, open) again. NEVER reintroduce a display-side correction of the number — the calm-line fence existed and was deliberately torn down on 24/08/2026 (Vai); rebuilding it needs a Miltos decision in the decision log, not a gate fix. If the shore figure itself is wrong, calibrate the x0,5 discount (the cause), not the screen.',
    command: process.execPath,
  ...
  {
    id: 'warning-label-coverage',
    title: 'Καμία προειδοποίηση της κάρτας δεν μιλάει αγγλικά σε Έλληνα',
    description: 'Διαβάζει τις τιμές του WarningFlagType από το types.ts (μοναδική πηγή) και ελέγχει ότι καθεμιά έχει δικό της case μέσα στη warningLabel του BeachCard, ότι το default μένει ως δίχτυ ασφαλείας, και ότι το ελληνικό μπλοκ δεν αντιγράφει αυτούσια καμία αγγλική πρόταση του κινητήρα.',
    protects: 'Το warning.message το γράφει ο κινητήρας για τον εαυτό του, στα αγγλικά. Όποιος τύπος δεν έχει case πέφτει στο default και τυπώνει αυτούσια την αγγλική πρόταση σε ελληνική, γαλλική, γερμανική και ιταλική κάρτα — χωρίς να σπάσει τίποτα. Στις 22/08/2026 έλειπαν 11 από τους 20 τύπους και κανένας έλεγχος δεν το έβλεπε.',
    failureAction: 'Γράψε κείμενο και στις ΠΕΝΤΕ γλώσσες στο cardCopy[…].warnings και πρόσθεσε το case. ΜΗΝ αντιγράψεις το αγγλικό message — γράφτηκε για μηχανή. Αν κουβαλάει νούμερο, πέρασέ το από το WarningFlag.values και κάνε τη λεζάντα συνάρτηση.',
    command: process.execPath,
    args: ['scripts/validateWarningLabelCoverage.mjs'],
  },
  {
    id: 'shore-shadow-contract',
    title: 'Η έκπτωση της ακτής είναι γωνία — μία, φραγμένη, και τη διαβάζουν όλοι από το score',
    description: 'Οδηγεί την πραγματική resolveShoreShadowDamping (σχήμα: διάδρομος→1, άκρη→0,5, e^(−θ/45°), δάπεδο 0,10, μονοτονία, κενά→undefined, και από 29/08 το πάτωμα της πλάγιας θάλασσας: πόρτα ≥10 χλμ + onshore>−0,65 → K_d ≥ 0,5) και τη shoreSeaStateM (K_d στο protected σκέλος ΚΑΙ στο enclosed της τσέπης, φραγμένο [0,1], το grazing του §Γ59 κρατά το 0,5 του), καρφώνει τους 6 αριθμούς, ελέγχει στον πηγαίο κώδικα ότι scoring/ταβάνι/πόρτα-4Μπφ/αιτία/πινέζα διαβάζουν ΟΛΟΙ το score.shoreShadowDamping, και αποδεικνύει με κλήση ότι η πινέζα με K_d=1 δεν βγαίνει πιο ήρεμη από K_d=0,1. Από 11/09/2026 δύο ακόμα: (Ε2) διαβάζοντας το συντακτικό δέντρο, κάθε αντικείμενο που αντιγράφει από score τη θάλασσα της σκάλας χρώματος μαζί με το τσιπ ή τον όρμο (App.tsx mapSuitableBeaches, οι δύο builders του recommendationService, ο μικρός χάρτης της σελίδας) πρέπει να αντιγράφει και το K_d και την άφιξη της θάλασσας, το τσιπ να έχει και να περνάει την παράμετρο, και κάθε ανά-παραλία κλήση της σκάλας στον χάρτη (πινέζα, ταμπελάκι πάνελ) να την περνάει· (Ζ) σε 68.544 ζεύγη (110 περιοχές, κάθε παραλία με προφίλ, 24 σενάρια όπου το K_d αλλάζει χρώμα) με το πραγματικό calculateBeachScore και τα πεδία που το App.tsx και η σελίδα ΠΡΑΓΜΑΤΙΚΑ αντιγράφουν, 0 διαφορές: πινέζα == πινέζα με το K_d του score, τσιπ (= χρώμα σελίδας) == τσιπ με το K_d του score, μικρός χάρτης σελίδας == πινέζα περιοχής, και όπου πινέζα και σελίδα κοιτάζουν την ίδια ακτή με τα ίδια δεδομένα ίδιο K_d και ίδιο χρώμα· και απαιτεί το δείγμα να ΒΛΕΠΕΙ ένα χαμένο K_d (αλλιώς θα περνούσε στα τυφλά).',
    protects: 'Στις 24/08/2026 το επίπεδο ×0,5 έγινε γωνιακό K_d (απόφαση Μίλτου, μετρημένο εθνικά: 73,6% των εκπτώσεων βαθιά σκιά, 394 παραλίες με έκπτωση πάνω σε κύμα από ανοιχτό διάδρομο — reports/quality/shadow-direction-sweep.json). Πέντε αρχεία διαβάζουν τον συντελεστή· αν ένα αποσυνδεθεί, κάρτα και πινέζα λένε διαφορετικό νερό (§Κ1/§Γ56), και αν το grazing σκέλος πάρει ποτέ K_d, ένα μοντέλο θα υπερισχύσει μετρημένης μαρτυρίας καμερών. Αυτό ακριβώς έγινε, αόρατα, για 18 μέρες: το Ε κοίταζε ότι ο χάρτης ΠΕΡΝΑΕΙ το πεδίο, όχι ότι το αντικείμενο το ΚΟΥΒΑΛΑΕΙ — τα αντικείμενα του App.tsx δεν το αντέγραφαν και το τσιπ δεν είχε καν όρισμα, οπότε πινέζα και τσιπ έμεναν στο ×0,5 ενώ η σελίδα διάβαζε K_d. Μετρημένο 11/09/2026 (scripts/measurePinShoreShadowWiring.mjs): 1.974 πινέζες και 1.234 τσιπ άλλαζαν χρώμα στο κύριο πλέγμα — 728 πινέζες «Ιδανική» πάνω από «Καλή» σελίδα — και 85.748 σε θάλασσα ≥2 μ.· και καλωδίωση ΜΟΝΟ της πινέζας θα έριχνε τη συμφωνία πινέζας-σελίδας από 99,5% σε 85,6%.',
    failureAction: 'Αν έπεσε το Ε, ξαναπέρασε το score.shoreShadowDamping στην επιφάνεια που αποσυνδέθηκε — μην την αφήσεις να υπολογίζει δικό της. Αν το Ε δείχνει αντικείμενο χωρίς K_d ή χωρίς άφιξη θάλασσας, πρόσθεσε ΚΑΙ ΤΑ ΔΥΟ από το ίδιο score: K_d χωρίς άφιξη δίνει έκπτωση εκεί που η πινέζα την αρνείται. Αν έπεσε το Ζ, το χρώμα της πινέζας, του τσιπ ή του μικρού χάρτη δεν είναι πια αυτό της σελίδας: διόρθωσε την καλωδίωση πινέζας ΚΑΙ τσιπ μαζί — ποτέ μόνο τη μία — και μη βγάλεις σενάριο από το δείγμα για να περάσει. Αν λέει ότι το δείγμα είναι τυφλό, άλλαξαν τα προφίλ ή η σκάλα: ξαναδιάλεξε σενάρια με το scripts/measurePinShoreShadowWiring.mjs. Αν έπεσε το Δ, οι 5 αριθμοί είναι απόφαση πάνω σε μέτρηση: νέα τιμή = ξανατρέξε measureShoreShadowPhysics/measureShadowDirectionSweep ΚΑΙ νέα απόφαση. Το grazing μένει 0,5 μέχρι να υπάρξει νέα μέτρηση με μάτια.',
    command: process.execPath,
    args: ['scripts/validateShoreShadowContract.mjs'],
  },
  {
    id: 'shore-shadow-witnesses',
    title: 'Οι εφτά μάρτυρες της σκιάς συμφωνούν με τις κάμερες της 29/08',
    description: 'Ξαναπαίζει μέσα από τον ΠΡΑΓΜΑΤΙΚΟ κινητήρα (calculateBeachScore + readout κάρτας), με τα κομμιταρισμένα προφίλ και καρφωμένο μελτέμι (24 χλμ/ώ Β, κελί 1,1 μ.), τους τέσσερις μάρτυρες των webcam της 29/08/2026 (Βάι, Κιτροπλατεία, Αλμυρός: η λέξη ανέβηκε από «λάδι»· Λίνδος: η τσέπη ηρέμησε στο K_d της) και τους τρεις μάρτυρες-έλεγχο (Πρέβελη, Κουκουναριές, Γέρακας: γνήσια απάνεμες, μένουν «θάλασσα λάδι» με τη βαθιά σκιά τους).',
    protects: 'Η μόνη αλήθεια για μια ακτή είναι μάτια και κάμερες. Στις 29/08 τέσσερις κάμερες διέψευσαν την οθόνη μέσα σε μία ώρα, όλες από το ίδιο σημείο: η έκπτωση σκιάς κρινόταν από γωνίες αντί για «πόσο νερό μπορεί να φτάσει εδώ». Χωρίς την πύλη, μια μελλοντική «βελτίωση» της σκιάς μπορεί να ξαναγράψει «θάλασσα λάδι» πάνω σε σπάσιμο κύματος — ή να αγριέψει τις γνήσια απάνεμες για να σώσει τις άλλες.',
    failureAction: 'ΜΗΝ αλλάξεις το σενάριο για να πρασινίσει. Τρέξε scripts/probeShoreWaveChain.mjs στον μάρτυρα που έσπασε και δες ποιο σκαλί γύρισε (K_d, άφιξη, ράμπα, λέξη). Αν άλλαξαν τα ψημένα προφίλ, ξαναμέτρησε με scripts/measureShoreShadowGate.mjs πριν δεχτείς τη νέα συμπεριφορά — οι κάμερες της 29/08 δεν αλλάζουν αναδρομικά.',
    command: process.execPath,
    args: ['scripts/validateShoreShadowWitnesses.mjs'],
  },
  {
  ...
  {
    id: 'manual-constants',
    title: 'Το εγχειρίδιο της βίβλου λέει τους αριθμούς που όντως τρέχουν',
    description: 'Διαβάζει 17 σταθερές από τον ΠΗΓΑΙΟ κώδικα (αποσυμπίεση ανέμου a/b, δάπεδος ριπής και λόγος ασυνέπειας, οι πέντε αριθμοί του K_d, η πόρτα των 3 Μποφόρ, το δάπεδο «Ιδανική», η ασήμαντη διαφορά του βάθρου, η περίοδος αναφοράς και ο εκθέτης απότομου κύματος, το δάπεδο του τυπωμένου αριθμού ακτής) και απαιτεί να εμφανίζονται αυτούσιες στο κεφάλαιο «ΤΟ ΕΓΧΕΙΡΙΔΙΟ» του docs/team/PORISMA-KAIROS-2026-08.md. Μετράει επίσης τέσσερα πλήθη από τα ίδια τα αρχεία (πύλες, παραλίες με κελί νερού, ids κλειστού νερού, σημεία δεύτερου θαλάσσιου μοντέλου), απαγορεύει έξι διατυπώσεις που ΗΤΑΝ σωστές και έγιναν ψευδείς, απαιτεί να υπάρχουν και τα οκτώ στάδια με τη σειρά τους, και σαμποτάρει τον εαυτό της σβήνοντας κάθε σταθερά μία-μία από αντίγραφο του κειμένου. Παραλείπεται σιωπηλά όπου λείπει η βίβλος (gitignored — δημόσιο repo).',
    protects: 'Το εγχειρίδιο υπόσχεται ρητά «αν αλλάξει ο κώδικας, αλλάζει ΑΥΤΟ το κεφάλαιο την ίδια μέρα» — και η υπόσχεση έσπασε δύο φορές. Στις 20/08/2026 η κεφαλίδα είχε μείνει πίσω από τις 17/08. Από 24/08 ως 10/09/2026 το ίδιο το εγχειρίδιο έμεινε πίσω 17 ημέρες σε πέντε σημεία, όλα βαριά: περιέγραφε δάπεδο ριπής που είχε αντικατασταθεί από τη γραμμική αποσυμπίεση (§Γ69), έλεγε «ΜΟΝΟ η διεύθυνση έρχεται από το νερό» ενώ από 25/08 ερχόταν και η ταχύτητα σε 1.833 παραλίες, έλεγε σταθερό ×0,5 έκπτωση ακτής ενώ είχε γίνει γωνία K_d(θ) (§Γ71), περιέγραφε ζωντανό τον φράχτη ζώνης που είχε γκρεμιστεί (§Γ70), και μετρούσε 67 πύλες ενώ ήταν 87. Το ημερολόγιο τα είχε όλα σωστά· μόνο το κεφάλαιο-περίληψη ξέμεινε — και αυτό είναι το μόνο που διαβάζει όποιος πιάνει δουλειά. Ένα ξεπερασμένο εγχειρίδιο είναι χειρότερο από κανένα: οδηγεί κάποιον να «διορθώσει» πίσω κάτι που μετρήθηκε εθνικά.',
    failureAction: 'ΜΗΝ χαλαρώσεις την πύλη και ΜΗΝ σβήσεις τον αριθμό από τη λίστα της. Άλλαξες σταθερά του μοντέλου: ενημέρωσε το κεφάλαιο «ΤΟ ΕΓΧΕΙΡΙΔΙΟ» στην ΙΔΙΑ συνεδρία και, αν η αλλαγή μετακινεί συμπεριφορά μετά το κλείδωμα της 05/08, γράψε την εξαίρεση ΚΑΙ στην κεφαλίδα της βίβλου. Το ημερολόγιο (§Γ…) δεν ξαναγράφεται ποτέ — προσθέτεις νέα ενότητα με τη μέτρηση, και βάζεις σημείωση ανατροπής στο § που ανατρέπεις.',
    command: process.execPath,
    args: ['scripts/validateManualConstants.mjs'],
  },
  {
  ...
  },
  {
    ...
    title: 'Ό,τι δεν φαίνεται στην οθόνη δεν αλλάζει χρώμα',
    description: "Έξι ισχυρισμοί γύρω από τις δύο αλλαγές της 24/08/2026. (Α) Το DISPLAYED_WAVE_STEP_M συμφωνεί με τα δεκαδικά που τυπώνει πραγματικά το utils/beachConditionsReadout — διαβασμένα από την πηγή, όχι αντιγραμμένα. (Β) Και τα τρία κατώφλια σοβαρότητας (0,40 · 0,80 · 1,20) είναι ακέραια πολλαπλάσια του βήματος, που είναι η ΒΑΣΗ της απόδειξης ότι η στρογγυλοποίηση μπορεί μόνο να αυστηροποιήσει. (Γ) Σε 201 τιμές 0–2,00 μ. ούτε το ταβάνι χρώματος ούτε η λέξη της ετυμηγορίας γίνονται ποτέ ηρεμότερα από την ωμή σύγκριση. (Δ) Δύο θάλασσες που τυπώνονται ολόιδια κρίνονται ολόιδια — και ρητά το ζευγάρι που το γέννησε, Τσερδάκια 0,80 vs Χρυσή Ακτή 0,78. (Ε) Το χρώμα και η λέξη στρογγυλοποιούν ΜΑΖΙ, ποτέ το ένα χωρίς το άλλο. (Ζ) Σε 432 συνδυασμούς έκθεσης × Μποφόρ × θάλασσας × ετυμηγορίας, το χρώμα της κάρτας δεν είναι ποτέ ηρεμότερο από το χειρότερο των δύο σημάτων της, και το σήμα του νερού βάφεται ακριβώς όταν η θάλασσα κατέβασε το χρώμα. Self-proves με --prove.",
    protects: 'Δύο πράγματα που έσπασαν μαζί στις 24/08/2026: γειτονικές παραλίες με ΤΑ ΙΔΙΑ τυπωμένα νούμερα φορούσαν διαφορετικό χρώμα (308 από 8.526 ζεύγη ≤8 χλμ, 3,6%), και τίποτα στην κάρτα δεν έδειχνε ποιο από τα δύο σήματα τις χώρισε. Η θεραπεία στηρίζεται σε μια ιδιότητα που σπάει ΣΙΩΠΗΛΑ: αν κάποιος προσθέσει κατώφλι εκτός πλέγματος (π.χ. 0,85) ή δείξει δεύτερο δεκαδικό, η στρογγυλοποίηση παύει να είναι μονόδρομη και μπορεί να κάνει μια παραλία να φαίνεται ΗΡΕΜΟΤΕΡΗ απ\' ό,τι είναι — η σκανδάλη #1 του §9. Και αν το χρώμα στρογγυλοποιήσει χωρίς τη λέξη, ξαναγεννιέται η αντίφαση «ήρεμα» κάτω από κίτρινη πινέζα που το utils/seaVerdict υπάρχει για να κλείσει.',
    failureAction: 'Αν έπεσε το Β, ΜΗΝ στρογγυλοποιήσεις το νέο κατώφλι για να περάσει: ή βάλ\' το πάνω στο πλέγμα, ή απόδειξε ξεχωριστά ότι η στρογγυλοποίηση παραμένει μονόδρομη γι\' αυτό. Αν έπεσε το Α, κάποιος άλλαξε πόσα δεκαδικά τυπώνει η οθόνη — το βήμα πρέπει να τον ακολουθήσει, αλλιώς η κρίση και η εμφάνιση ξαναχωρίζουν. Αν έπεσε το Ε, μπήκε στρογγυλοποίηση στο χρώμα και ξεχάστηκε στη λέξη (ή αντίστροφα). Αν έπεσε το Ζ, κάποια επιφάνεια υπολογίζει μόνη της τα δύο σήματα αντί να τα διαβάζει από το SimpleWindSuitability.',
    command: process.execPath,
    args: ['scripts/validateDisplayedPrecisionGate.mjs'],
  },
  {
    id: 'over-water-wind-layer',
    title: 'Το στρώμα ανέμου πάνω από νερό αγγίζει διεύθυνση και ταχύτητα — ποτέ τη ριπή',
    description: 'Οδηγεί την πραγματική applyOverWaterWind χωρίς καμία κλήση δικτύου. Επτά ισχυρισμοί (ξαναγράφτηκε 25/08/2026 όταν μπήκε και η ΤΑΧΥΤΗΤΑ, §Γ51/§Γ52): (Α) η πύλη των 3 χλμ ζει στα ΔΕΔΟΜΕΝΑ — καμία παραλία κάτω από το όριο δεν έχει κελί θάλασσας στο data/forecast-sea-cells.generated.json και καμία πάνω από αυτό δεν λείπει· (Β) σε 1.728 συνδυασμούς η ταχύτητα γίνεται ΑΚΡΙΒΩΣ του νερού, η στεριανή μένει στο speedBeforeOverWater, ριπή και ωμός στεριανός μέσος βγαίνουν πανομοιότυποι· (Γ) κάτω από 3 Μποφόρ (στο νούμερο που τυπώνεται) η διεύθυνση δεν κουνιέται, η ταχύτητα ΚΟΥΝΙΕΤΑΙ, και όταν το νερό σηκώσει την ώρα πάνω από την πύλη η διεύθυνση γυρίζει· (Δ) ο ημερήσιος άνεμος μένει ΤΟ ΙΔΙΟ αντικείμενο με μια ώρα της ίδιας πρόγνωσης· (Ε) αυτοσαμποτάζ — το στρώμα ΜΠΟΡΕΙ να αλλάξει και τα δύο, ο ελεγκτής έντασης ΜΠΟΡΕΙ να πει όχι, και ίδιες τιμές δίνουν ΤΟ ΙΔΙΟ αντικείμενο· (ΣΤ) ώρα χωρίς ταχύτητα νερού κρατά τη στεριανή· (Ζ) τα spreads ριπής στο recommendationService πέφτουν πίσω στη στεριανή ταχύτητα, όχι στη θαλάσσια.',
    ...
    failureAction: 'Αν έπεσε το Η (Δ9-Α, 13/09/2026), το ledger `data/sea-wind-cell-overrides.json` και ο χάρτης διαφωνούν: `node scripts/bakeSeaWindCells.mjs --apply-overrides` και μετά `node scripts/buildBeachRegionData.mjs && node scripts/buildBeachGeoIndex.mjs` — ΜΗΝ σβήσεις εγγραφή του ledger για να περάσει (κάθε εγγραφή στέκεται πάνω σε DEM ≥200 μ. και echo της πόρτας). Αν έπεσε το Α, ο ψημένος χάρτης έχει μπαγιατέψει: ξαναψήσ᾽ τον με `npm run bake:sea-cells` — ΜΗΝ χαλαρώσεις την πύλη για να περάσει. Αν έπεσε το Β, κάποιος έβαλε το στρώμα να πειράζει και την ΤΑΧΥΤΗΤΑ: το §Γ29 μέτρησε ότι η θάλασσα ΧΑΝΕΙ εκεί σε δύο ανεξάρτητα παράθυρα, και κάτω από 3 χλμ χάνει σταθερά. Αν έπεσε το Γ, η έκπτωση των ~30% των κλήσεων χάθηκε και μαζί ο λόγος που ο Μίλτος ενέκρινε τη δουλειά. Αν έπεσε το Δ, η κάρτα και η πινέζα διαβάζουν πια άλλη διεύθυνση για την ίδια ώρα — το ακριβές λάθος που κυνήγησε το §Γ27.',
    command: process.execPath,
    args: ['scripts/validateOverWaterWindLayer.mjs'],
  },
  {
    id: 'verdict-wave-monotonicity',
    title: 'Μικρότερο κύμα δεν βγάζει ποτέ αυστηρότερη λέξη',
    description: 'Οδηγεί την ΠΡΑΓΜΑΤΙΚΗ μηχανή (calculateBeachScore) πάνω σε σκάλα υψών κύματος ανά 1 εκατοστό, από 0,10 ως 1,60 μ., με όλα τα υπόλοιπα σταθερά, σε 225 περιβάλλοντα (3 επίπεδα έκθεσης × 5 εντάσεις ανέμου × 3 περίοδοι × 5 γωνίες κύματος — μαζί με θάλασσα που ΦΕΥΓΕΙ από την ακτή). 33.975 κλήσεις. Απαιτεί μονοτονία: καθώς το κύμα ανεβαίνει, η ετυμηγορία κολύμβησης δεν γίνεται ΠΟΤΕ ηπιότερη. Αυτοσαμποτάζ με --prove: μια σκόπιμα ανάποδη συνάρτηση ΠΡΕΠΕΙ να τη ρίξει.',
    protects: 'Νήμα 16: στη μέτρηση της ξυστής θάλασσας (06/09) φάνηκαν 3 ώρες στο Πρασονήσι #2457 όπου λιγότερο κύμα έδινε ΧΕΙΡΟΤΕΡΗ συμβουλή. Μετρήθηκε 14/09 ότι η μηχανή ΕΙΝΑΙ μονότονη ως προς το ύψος (33.975 βήματα, καμία αντιστροφή) — οι 3 ώρες ήταν σύγκριση δύο ΠΑΡΑΛΛΑΓΩΝ του κανόνα, όχι δύο υψών. Η πύλη κλειδώνει το αναλλοίωτο για το μέλλον: η ετυμηγορία περνάει από πολλούς τελεστές (δάπεδο, ταβάνι, ανακούφιση υπερβολικής προσοχής, άφιξη θάλασσας) και ο επόμενος που θα προστεθεί μπορεί εύκολα να γυρίσει τη φορά χωρίς να το δει κανείς.',
    failureAction: 'ΜΗΝ χαλαρώσεις την απαίτηση. Κάθε αντιστροφή τυπώνεται με τα ακριβή της νούμερα (έκθεση, άνεμος, περίοδος, γωνία, δύο ύψη, δύο λέξεις): αναπαρήγαγε ακριβώς αυτή την περίπτωση και βρες ΠΟΙΟΣ τελεστής διαβάζει το ύψος και γυρίζει ανάποδα. Συνήθης ύποπτος: ένα κατώφλι που ανοίγει μια ανακούφιση μόνο μέσα σε ζώνη υψών.',
    command: process.execPath,
    args: ['scripts/validateVerdictWaveMonotonicity.mjs', '--prove'],
  },
  {
    id: 'cove-chop-floor-cap',
    title: 'Το δάπεδο ψιλοκύματος κόβεται στους όρμους — και το fetch φτάνει ως εκεί',
    description: 'Τρέχει τη capWindChopFloorByFetchM και τον resolveDisplayWaveHeightM σε πλέγμα όρμων (4-7 Μπφ, 0-2,9 χλμ, εκτεθειμένη/μερική): κάτω από 3 χλμ το δάπεδο πέφτει και ποτέ κάτω από την «προστατευμένη», ≥3 χλμ / χωρίς fetch / protected αμετάβλητο, ποτέ πάνω από το αρχικό. Μετά διαβάζει το services/recommendationService.ts και απαιτεί ΚΑΘΕ κλήση του resolver να περνά windSectorFetchKm: windAssessment.effectiveFetchKm. Αυτοσαμποτάζ: με το πέρασμα σβησμένο από έναν καλούντα πρέπει να πέσει.',
    protects: 'Από 14/09/2026 (§Γ81 Δ6-Γ, απόφαση Μίλτου Α) το 0,80 μ. των 5 Μποφόρ δεν τυπώνεται σε όρμο με < 3 χλμ νερό: ο δορυφόρος (5 καλοκαίρια, 252 μέρες) είδε αφρό θραύσης 2% εκεί, έναντι 23% με πραγματικό κύμα. Η αλλαγή έχει δύο μισά που σπάνε αόρατα — τη συνάρτηση και το ΠΕΡΑΣΜΑ του fetch από τους δύο καλούντες. Αν κάποιος σβήσει το windSectorFetchKm, κάθε όρμος ξαναγίνεται κίτρινος στο μελτέμι και καμία άλλη πύλη δεν το βλέπει.',
    failureAction: 'Αν έπεσε το Γ, ξαναβάλε windSectorFetchKm: windAssessment.effectiveFetchKm στην κλήση που το έχασε. Αν έπεσε το Α/Β, ΜΗΝ χαλαρώσεις το κατώφλι της πύλης: η μόνη δεκτή αλλαγή στο COVE_FETCH_KM ή στο ελάχιστο είναι με νέα μέτρηση δορυφόρου (scripts/judgeChopFloorSentinel2.mjs) και απόφαση.',
    command: process.execPath,
    args: ['scripts/validateCoveChopFloorCap.mjs'],
  },
  {
  ...
  },
  {
    ...
    failureAction: 'Τρέξε `node scripts/auditMarineCellTrust.mjs --apply` για να ξαναμετρηθεί και να ξαναμπεί η σήμανση. ΠΟΤΕ μην ανεβάσεις το UNTRUSTED_BASELINE για να περάσει η πύλη: ο αριθμός επιτρέπεται μόνο να πέφτει, και μια αύξηση σημαίνει ότι κάτι μετακίνησε σημεία θάλασσας ή ότι μπήκαν παραλίες σε κακή θέση. Και μη σβήσεις τη σημαία για να «καθαρίσουν» τα δεδομένα — από 22/08 (βίβλος §Γ64) τη ΔΙΑΒΑΖΕΙ η εκτέλεση: οι σημαδεμένες βγαίνουν από το βάθρο τις μέρες με θάλασσα ≥0,5 μ., οπότε μια σβησμένη σημαία θα ξανάβαζε στις προτάσεις παραλίες που διαβάζουν ξένο νερό.',
    command: process.execPath,
  ...
  },
  {
    ...
    description: 'Drives every wind × wave × language combination through the card\'s one-line description and checks four things: it fits two lines on a 320 px phone, since 22/08 (bible §Γ66) the card shows WORDS only, so each word must fit half a tile (21/18 characters) and ≥7 Bft must read red and never enter the list, no verdict word ("ideal", "avoid") can enter a line that is meant to describe rather than judge, and the relief connective ("but") stops at 5 Bft so a red-pinned beach never reads like good news. Also re-checks the top band still shares its exact wording with the map legend in all five languages.',
    protects: 'Prevents the two readings on the card (numbers «5 Μπφ | ~0,1 μ.» until 22/08, words since — bible §Γ66) from meaning nothing to the reader, and the sentence that translates them from contradicting them. Twice in 24 hours (13/08, 14/08/2026) a correct description beside a correct number was clipped on a phone, and the earlier defect class is worse: a word that describes the OPEN sea sitting above a shore figure would print «big waves» over «~0,1 μ.».',
  ...
  },
  {
    ...
    description: 'Τρέχει την ΑΛΗΘΙΝΗ `resolveCalmWaterState` (utils/calmWaterFilter) σε 8 κατασκευασμένα σενάρια που τρέχουν πάντα, και — όταν υπάρχει το εθνικό δείγμα στο .tmp — στις 550 σκηνές (110 περιοχές × 5 μέρες). Τέσσερις ισχυρισμοί: (Α) όποτε προσφέρεται το chip, ο αριθμός του είναι >0 και <όλες, και η λίστα του έχει ακριβώς τόσα μέλη· (Β) καμία παραλία με `avoid_swimming` ή απόγειο-γυαλί δεν μπαίνει ποτέ μέσα, και καμία με κύμα ≥0,4 μ.· (Γ) κάτω από 3 Μποφόρ το chip σιωπά, και σιωπά ΓΙ᾿ ΑΥΤΟΝ τον λόγο· (Δ) από 23/08 (βίβλος §Γ67) δεν κρίνεται σε απόλυτο 20%: Δ1 προσφέρεται σε ≥1/3 των σκηνών όπου επιτρέπεται, Δ2 επιτρέπεται σε ≥10%, Δ3 καμία από τις σκόπιμες σιωπές δεν έπεσε στο μηδέν.',
  ...
  {
    id: 'landing-shelter-bound',
    title: 'Η landing δεν υπόσχεται περισσότερες προστατευμένες απ᾽ όσες δείχνει ο χάρτης',
    description:
      'Παίρνει το ψημένο πλήθος προστατευμένων παραλιών κάθε περιοχής (data/landingShelter.generated.json) και το αντιπαραβάλλει με το πλήθος που βγάζει το ΙΔΙΟ assessBeachWindExposure που χρωματίζει τον χάρτη, σε 24 κατευθύνσεις × 9 εντάσεις ανέμου. Απαιτεί ψημένο ≤ πραγματικό παντού, και επιπλέον ότι το αρχείο είναι αυτό που παράγουν τα σημερινά δεδομένα παραλιών.',
    protects:
      'Τα πλακίδια της landing έδειχναν ζωντανά Μποφόρ ώσπου μετρήθηκε ότι το σημείο δειγματοληψίας (ανοιχτή θάλασσα) διαφωνεί με την ακτή που διαβάζει η σελίδα της περιοχής — δύο αριθμοί για το ίδιο μέρος την ίδια μέρα, reports/region-forecast-point-audit.md. Το πλήθος προστατευμένων επαναφέρει έναν αριθμό στα πλακίδια και ξαναφέρνει τον ίδιο κίνδυνο, με μία διαφορά: εδώ είναι ΚΑΤΩ ΦΡΑΓΜΑ, ψημένο στα 6 Μποφόρ. Η πρώτη γραφή μετρούσε με σκέτη γεωμετρία και διαφωνούσε στο 7,64% των παραλιών — στη Ρόδο 44 έναντι 27. Χωρίς αυτή την πύλη, η ίδια απόκλιση επιστρέφει σιωπηλά με την επόμενη αλλαγή στο μοντέλο.',
    failureAction:
      'Αν έπεσε το φράγμα, μια αλλαγή στο μοντέλο έκθεσης έκανε κάποιες παραλίες λιγότερο προστατευμένες απ᾽ ό,τι υπόσχεται η landing: ξαναψήσε με node scripts/buildLandingShelter.mjs. ΠΟΤΕ μην ανεβάσεις το BAKE_BEAUFORT για να περάσει — το φράγμα είναι η υπόσχεση, όχι η ρύθμιση.',
    command: process.execPath,
    args: ['scripts/validateLandingShelterBound.mjs'],
  },
  {
    id: 'sitemap-lastmod',
