...
- Mobile condition cards must avoid long explanatory subtext when the primary verdict already answers the question. For mild sea states, show only the short sea verdict instead of a sentence that can be clipped on small screens.
- Mobile feature/access chips must use short labels that fit fixed-width pills. Keep full access wording for detail sections; compact Greek labels like dirt-road access should not use long words that break or get clipped.
- Mobile metric rows inside cards should distribute available items across the full frame. Do not left-pack wind/wave/temperature values in a flex row that leaves dead space at the end.
