...
export interface FeedbackData {
  ...
  conditions?: {
     ...
     *
     * Το `live` από πάνω είναι `selectedDayIsToday && selectedHour === undefined`, δηλαδή
     * σβήνει και όταν αλλάξει η ΜΕΡΑ και όταν αλλάξει η ΩΡΑ, χωρίς να λέει ποιο από τα δύο.
     * Κολοκύθα 747, 29/08/2026: ο επισκέπτης ήταν εκεί το απόγευμα και η σελίδα ήταν γυρισμένη
     * στις 10:00 της ΕΠΟΜΕΝΗΣ μέρας — το μήνυμα έγραφε «γυρισμένη σε άλλη ώρα» και διαβαζόταν
     * ως ίδια μέρα, δηλαδή ως σχεδόν συγκρίσιμη αναφορά.
     *
     * Η διαφορά δεν βγαίνει από το `date`: εκείνο γράφεται με το ρολόι ΤΗΣ ΣΥΣΚΕΥΗΣ, οπότε
     * ένας δέκτης που το συγκρίνει με τη σημερινή ημερομηνία Ελλάδας θα έβγαζε λάθος
     * απάντηση για επισκέπτη σε άλλη ζώνη. Μόνο η σελίδα ξέρει σίγουρα, οπότε το στέλνει.
     */
