...
// landing hero's living sea AND the "seas today" panel (live conditions per sea
// area). ONE Open-Meteo request covers every sample point (comma-joined coords,
// exactly the pattern scripts/windSpreadNational.mjs already uses), cached for 3h
// because the national picture changes slowly — so this costs ~1 call per
// session, honouring the forecast-caching-safety work. The service worker also
// caches api.open-meteo.com. On any failure we return null and everything falls
// back to calm / "unavailable" — we never fabricate conditions.
