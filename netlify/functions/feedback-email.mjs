...
const formatLive = (conditions) => {
  ...
  // Η ΜΕΡΑ ΠΡΩΤΑ, ΚΑΙ ΜΟΝΟ ΟΤΑΝ ΤΟ ΞΕΡΟΥΜΕ (Κολοκύθα 747, 29/08/2026). Η σελίδα ήταν γυρισμένη
  // στις 10:00 της ΕΠΟΜΕΝΗΣ μέρας ενώ ο επισκέπτης ήταν εκεί το απόγευμα, και η γραμμή έγραφε
  // «γυρισμένη σε άλλη ώρα» — που διαβάζεται ως ίδια μέρα, δηλαδή ως αναφορά που σχεδόν
  // αντιπαρατίθεται. Δεν είναι: όταν αλλάζει η μέρα, τα νούμερα είναι πρόβλεψη για άλλο
  // εικοσιτετράωρο και το σχόλιο δεν βαθμονομεί τίποτα. Η ημερομηνία μπαίνει μέσα στη γραμμή
  // επειδή η γραμμή «Ημερομηνία» είναι εννιά σειρές πιο πάνω και δεν διαβάζεται μαζί.
  if (conditions.selectedDayIsToday === false) {
    const day = conditions.date ? ` (${conditions.date})` : '';
    return typeof conditions.shownHour === 'number'
      ? `Όχι — η σελίδα έδειχνε ΑΛΛΗ ΜΕΡΑ${day}, στις ${String(conditions.shownHour).padStart(2, '0')}:00`
      : `Όχι — η σελίδα έδειχνε ΑΛΛΗ ΜΕΡΑ${day}`;
  }
  // Ίδια μέρα, γυρισμένη ώρα — η μόνη περίπτωση όπου το «άλλη ώρα» είναι όντως όλη η αλήθεια.
  if (conditions.selectedDayIsToday === true) {
    return 'Όχι — ίδια μέρα, αλλά η σελίδα ήταν γυρισμένη σε άλλη ώρα (δες «Ώρα που έδειχνε η οθόνη»)';
  }
  // Χωρίς `selectedDayIsToday` (σχόλιο από παλιότερη έκδοση της σελίδας) δεν ξέρουμε αν άλλαξε
  // η μέρα ή η ώρα. Εκεί κρατάμε το παλιό, ασαφές αλλά τίμιο κείμενο· ένα σίγουρο «άλλη μέρα»
  // θα ήταν εφεύρεση, και σε γραμμή που ήδη διαβαζόταν λάθος η εφεύρεση είναι το ίδιο λάθος.
  return typeof conditions.shownHour === 'number'
    ? 'Όχι — η σελίδα ήταν γυρισμένη σε άλλη μέρα ή ώρα (δες «Ημερομηνία» και «Ώρα που έδειχνε η οθόνη»)'
...
const normalizePayload = (body, event) => {
  ...
  return {
    ...
    conditions: {
      ...
      // Χωρίς αυτό το `live: false` δεν λέει ΠΟΙΟ από τα δύο άλλαξε — μέρα ή ώρα. Το
      // normalizePayload κρατάει μόνο ό,τι απαριθμείται εδώ, οπότε ένα πεδίο που στέλνει η
