...
/**
 * ΤΟ ΔΑΠΕΔΟ ΕΦΑΡΜΟΣΜΕΝΟ — Ο ΚΑΝΟΝΑΣ ΑΠΟ ΠΑΝΩ ΗΤΑΝ ΓΡΑΜΜΕΝΟΣ ΚΑΙ ΔΕΝ ΕΤΡΕΧΕ (25/08/2026).
 *
 * Η ΑΝΑΦΟΡΑ: Λυγιά Λευκάδας #1165, 09:00. Επισκέπτης στην παραλία, η κάρτα έγραφε
 * «2 Μπφ · 0,0 μ. — Θάλασσα λάδι». Ο άνεμος ΗΤΑΝ όντως άπνοια (2-4 χλμ/ώρα σε τέσσερα
 * ανεξάρτητα μοντέλα) και καμία αποθαλασσιά δεν ανέβαινε τον διάδρομο Ν-ΝΑ — δηλαδή το
 * μοντέλο δεν έσφαλε. Έσφαλε ο ΤΥΠΩΜΕΝΟΣ αριθμός: ένα απόλυτο μηδέν διαβάζεται ως χαλασμένη
 * ένδειξη, όχι ως ήρεμο νερό, και αρκεί ένα απόνερο βάρκας για να το διαψεύσει.
 *
 * ΓΙΑΤΙ ΤΟ ΔΑΠΕΔΟ ΔΕΝ ΕΠΙΑΝΕ. Υπήρχε σε τρία σημεία και κανένα δεν κάλυπτε αυτή τη διαδρομή:
 * το `shoreWave` το βάζει (γρ. 570) αλλά αμέσως μετά σιωπά όταν το 0,10 βγει ≥ από το ανοιχτό
 * νερό (γρ. 582), οπότε ο αριθμός πέφτει στο «ανοιχτό × K_d» χωρίς πάτο (Λυγιά: 0,02 × 0,1);
 * και το δάπεδο πιο κάτω σε αυτό το αρχείο ζει μέσα σε `Math.min`, δηλαδή φρενάρει το ΤΑΒΑΝΙ,
 * δεν ανεβάζει χαμηλό αριθμό.
 *
 * ΜΕΤΡΗΘΗΚΕ ΠΡΙΝ ΜΠΕΙ (25/08/2026, 5 περιοχές, 1.441 ώρες-παραλίας): 406 = 28,2% τύπωναν
 * σκέτο «0,0 μ.», 61 ξεχωριστές παραλίες — χειρότερα σε κλειστούς κόλπους (Αιτωλοακαρνανία
 * 212/550, Λευκάδα 112/363, Πειραιάς 35/55).
 *
 * ⚠️ ΜΟΝΟ ΟΘΟΝΗ, ΚΑΙ ΠΡΕΠΕΙ ΝΑ ΜΕΙΝΕΙ ΕΤΣΙ. Μπαίνει ΜΟΝΟ εκεί που ο αριθμός τυπώνεται
 * (`utils/beachConditionsReadout` για κάρτα+πινέζα, το πλακίδιο θάλασσας στο
 * `pages/BeachDetailPage`). ΠΟΤΕ στο `shoreWaveM` του `services/recommendationService`: εκεί η
 * ΙΔΙΑ μεταβλητή τροφοδοτεί το `comfortForShoreHeight`, δηλαδή θα κουνούσε ετυμηγορία. Χρώμα
 * και ετυμηγορία διαβάζουν `beachDecisionSeaStateM` / `shoreWaveHeightM` και δεν τα αγγίζει.
 *
 * Η λέξη δεν αλλάζει: `conditionsFeelPhrase.WAVE_GLASSY_M` = 0,2, οπότε στα 0,10 μένει
 * «θάλασσα λάδι» — ο επισκέπτης παίρνει την ίδια ανάγνωση, χωρίς την αδύνατη ακρίβεια.
 */
export const printedWaveHeightM = (metres: number | undefined): number | undefined => (
  typeof metres === 'number' && Number.isFinite(metres)
    ? Math.max(metres, WAVE_DISPLAY_FLOOR_M)
    : undefined
);
...
export const resolveDisplayWaveHeightM = ({
  ...
  // όπου δηλώνει το πλέγμα ότι έρχεται το κύμα βρίσκει στεριά, πέτα την ένδειξη». Αφορμή ήταν η
  // Λυγαριά Ηρακλείου — 1,38 μ. από 336°, με στεριά στα 200 μ. προς τα εκεί. Η εθνική μέτρηση το
  // σκότωσε: άναβε σε 20.311 από 40.166 ώρες και γύριζε 2.217 χρώματα πινέζας προς το ηρεμότερο,
  // επειδή ΚΑΘΕ παραλία έχει στεριά στη μισή πυξίδα. Κάθε αυστηρότερη ρύθμιση που έριχνε τον
  // αντίκτυπο έπαυε να διορθώνει και τη Λυγαριά — το στόμιο του όρμου της απέχει μόλις ~45° από τη
  // γωνία του κύματος, δηλαδή η γεωμετρία ΔΕΝ μπορεί να ξεχωρίσει «μπαίνει» από «δεν μπαίνει».
  // Πλήρης καταγραφή και τα νούμερα: βίβλος §Μ6. Μην το ξαναγράψεις χωρίς νέα μέτρηση.
