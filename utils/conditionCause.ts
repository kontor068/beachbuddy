...
/**
 * ΤΑ ΔΥΟ ΣΗΜΑΤΑ ΤΗΣ ΚΑΡΤΑΣ, ΤΟ ΚΑΘΕΝΑ ΣΤΟ ΔΙΚΟ ΤΟΥ ΧΡΩΜΑ (24/08/2026).
 *
 * «Υπάρχουν πινέζες που βάφεται η μία μπλε κι η άλλη κίτρινη, και μποφόρ και κύμα τα γράφεις τα
 * ίδια» (Μίλτος, 24/08/2026 — Τσερδάκια vs Χρυσή Ακτή, 3 Μπφ και «0,1 μ.» και στις δύο).
 * Μετρημένο εθνικότερα την ίδια μέρα (10 περιοχές, 3.984 παραλιο-ώρες): από 8.526 ζεύγη γειτόνων
 * ≤8 χλμ που τυπώνουν ΤΑ ΙΔΙΑ νούμερα, **308 (3,6%) έχουν διαφορετικό χρώμα** — 38% από τη σκάλα
 * του ανέμου, 26% από το ταβάνι ετυμηγορίας, 17% από το δάπεδο ΙΔΑΝΙΚΗ, 4% από το ταβάνι θάλασσας.
 *
 * Η κάρτα δείχνει ΗΔΗ δύο ξεχωριστά σήματα με δικό τους εικονίδιο (components/BeachCard:
 * 💨 «Λίγος αέρας» · 🌊 «σχεδόν χωρίς κύμα») και τα ζωγραφίζει ΚΑΙ ΤΑ ΔΥΟ σταθερά γαλάζια, ό,τι
 * κι αν λένε. Έτσι ο αναγνώστης βλέπει δύο κάρτες με πανομοιότυπη εμφάνιση και διαφορετικό χρώμα
 * κάρτας, χωρίς τίποτα να δείχνει ΠΟΙΟ από τα δύο τις χώρισε. Αυτό δίνει σε κάθε σήμα το χρώμα
 * του δικού του παράγοντα, ώστε η απάντηση να διαβάζεται με μια ματιά αντί να εξηγείται με λέξεις.
 *
 * ΔΕΝ ΕΙΝΑΙ ΔΕΥΤΕΡΗ ΣΚΑΛΑ, ΚΑΙ ΔΕΝ ΕΠΙΤΡΕΠΕΤΑΙ ΝΑ ΓΙΝΕΙ. Ακριβώς η τεχνική του
 * `resolveConditionCause` από πάνω: καλεί το ΙΔΙΟ `resolveConditionTone` που βάφει την πινέζα και
 * απλώς κρύβει έναν παράγοντα τη φορά. Κανένα κατώφλι, καμία εξαίρεση όρμου και καμία αριθμητική
 * ταβανιού δεν αντιγράφεται εδώ — άρα δεν υπάρχει τίποτα να ξεκολλήσει από τη σκάλα αργότερα.
 *
 * ΤΟ ΝΕΡΟ ΒΑΦΕΤΑΙ ΜΟΝΟ ΟΤΑΝ ΚΑΤΕΒΑΣΕ ΤΟ ΧΡΩΜΑ, όχι όποτε υπάρχει θάλασσα. Το ταβάνι της
 * θάλασσας δεν έχει καμία γνώμη κάτω από SEA_STATE_AMBER_M, οπότε «δεν μίλησε» και «είναι ήρεμα»
 * είναι το ίδιο πράγμα — και ένα κίτρινο κυματάκι δίπλα στη λέξη «σχεδόν χωρίς κύμα» θα ήταν
 * αντίφαση στην ίδια γραμμή. Ο πραγματικός `beaufort` και ο όρμος περνιούνται αυτούσια, ώστε η
 * εξαίρεση του κλειστού όρμου να ισχύει εδώ ακριβώς όπως ισχύει στην πινέζα.
 *
 * Η ΕΤΥΜΗΓΟΡΙΑ ΚΑΙ ΤΟ ΦΡΕΝΟ ΜΕΝΟΥΝ ΕΞΩ ΚΑΙ ΑΠΟ ΤΑ ΔΥΟ. Δεν είναι ούτε αέρας ούτε κύμα: όταν
 * αυτά βάφουν, τα δύο σήματα λένε σωστά «ήρεμα» και το «μην κολυμπήσεις» της κάρτας — που
 * τυπώνεται ήδη ξεχωριστά — είναι το σήμα που εξηγεί το χρώμα.
 */
export interface FactorTones {
  /** Τι θα ήταν το χρώμα αν η θάλασσα ήταν λάδι — ο άνεμος πάνω σε ΑΥΤΗ την ακτή. */
  wind: CalmnessTone;
  /** Πόσο κατέβασε το χρώμα η θάλασσα· 'blue' όταν δεν το κατέβασε καθόλου. */
  sea: CalmnessTone;
}

export const resolveFactorTones = (input: ConditionCauseInput): FactorTones => {
  const base = { ...input, swimVerdictAvoid: false, forecastUncertain: false };
  const wind = resolveConditionTone({ ...base, seaStateM: undefined });
  const withSea = resolveConditionTone(base);
  return { wind, sea: isRougher(withSea, wind) ? withSea : 'blue' };
};
