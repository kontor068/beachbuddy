 ...
 * της ράμπας onshore) ΔΕΝ εγκρίθηκε.
 *
 * ✅ ΤΟ fan-all ΞΑΝΑΜΕΤΡΗΘΗΚΕ ΣΕ ΜΕΛΤΕΜΙ ΚΑΙ ΑΠΟΡΡΙΦΘΗΚΕ ΟΡΙΣΤΙΚΑ (21/08/2026, §Γ46/§Γ49).
 * Η μέτρηση της 18/08 είχε γίνει σε άπνοια 2-12 χμ/ώ, όπου η βεντάλια δεν έχει τι να απλώσει· με
 * το αρχείο προγνώσεων (§Γ45) η μέρα επιλέχθηκε (2022-09-06, 7 Μποφόρ). Σβήνει έως **1,69 μ.**,
 * και οι 8 από 8 πάνω από 1,0 μ., 6 από αυτές «κλάση Πανόρμου» — και γυρίζει «μην κολυμπήσεις»
 * σε «προσοχή» στην Κυριακούλου Κυθήρων #147, σε ΚΑΙ ΤΙΣ ΤΡΕΙΣ παραλλαγές. Μην το ξανανοίξεις
 * χωρίς νέα μέτρηση: η ερώτηση δεν είναι ανοιχτή, είναι απαντημένη.
 *
...
/**
 * ΠΟΣΟ ΛΟΞΑ ΕΠΙΤΡΕΠΕΤΑΙ ΝΑ ΦΕΥΓΕΙ ΤΟ ΝΕΡΟ — ΜΟΝΟ ΓΙΑ ΤΑ ΣΥΣΤΑΤΙΚΑ ΤΗΣ ΘΑΛΑΣΣΑΣ (22/08/2026).
 *
 * Ο ΑΝΕΜΟΣ ΜΕΝΕΙ ΣΤΟ −0,80 (`OFFSHORE_FLAT_MAX_ONSHORE`). Αυτή η σταθερά αφορά ΑΠΟΚΛΕΙΣΤΙΚΑ τη
 * γωνία με την οποία ταξιδεύει το ΝΕΡΟ. Είναι δύο διαφορετικά ερωτήματα και δεν είχαν λόγο να
 * μοιράζονται νούμερο: ο άνεμος πρέπει να είναι σίγουρα απόγειος γιατί χτίζει κύμα ΜΠΡΟΣΤΑ στην
 * ακτή που το κελί δεν έχει δει· η θάλασσα απλώς πρέπει να αποδεδειγμένα φεύγει.
 *
 * Η ΑΦΟΡΜΗ. Ελαφονήσι (595), 22/08/2026 13:00-16:00, χρήστης στην παραλία: «λάδι». Η κάρτα
 * έλεγε 0,9 μ. · «Αρκετό κύμα» · «μην κολυμπήσεις» (swimScore 33). Ο άνεμος ήταν 25 χλμ/ώ από
 * 312° — onshore −0,889, καθαρά απόγειος, ΠΕΡΝΑΕΙ την πύλη. Το κύμα 0,88 μ. από 293° — onshore
 * −0,69, και το −0,80 το έκοβε. Ο κανόνας που γράφτηκε ΓΙ' ΑΥΤΗΝ ΤΗΝ ΠΑΡΑΛΙΑ στις 16/08 αστοχούσε
 * κατά 0,11 στον έναν από τους δύο άξονές του.
 *
 * ΜΕΤΡΗΘΗΚΕ ΣΕ ΤΕΣΣΕΡΑ ΠΑΡΑΘΥΡΑ, ΟΧΙ ΕΝΑ (scripts/measureDepartingSeaThreshold.mjs · 2.780
 * παραλίες με προφίλ 'high' × 14 ώρες κολύμβησης × 4 μέρες ≈ 155.500 ώρες×παραλία), με τα
 * μοντέλα που ΟΝΤΩΣ διαβάζει η εφαρμογή (`models=ewam`). Παραλλαγή «άνεμος σταθερά ≤ −0,80»:
 *
 *   κατώφλι νερού   ώρες×παρ   παραλίες   μέγιστη πτώση   ≥1,00 μ. → δάπεδο
 *     −0,75             22          9          1,31 μ.            0
 *     −0,70             43         13          1,31 μ.            0
 *     −0,65             74         20          1,31 μ.            0
 *     −0,60             94         24          1,31 μ.            2   ⛔
 *
 * ΤΟ −0,65 ΕΙΝΑΙ ΤΟ ΤΕΛΕΥΤΑΙΟ ΜΕ ΜΗΔΕΝ ΚΑΤΑΡΡΕΥΣΕΙΣ ΣΤΟ ΔΑΠΕΔΟ, και στις τέσσερις μέρες. Στο
 * −0,60 εμφανίζονται οι πρώτες (14/08/2025) — αυτό είναι ακριβώς η σκανδάλη #1 της §9 και η
 * γραμμή σταματάει εκεί. Και στις 74 ώρες×παραλία του −0,65 ο τομέας είναι **'protected'** και ο
 * άνεμος μεταξύ −0,80 και −1,00: μηδέν 'partial', μηδέν 'exposed'. Δηλαδή δεν ανοίγει καινούργια
 * οικογένεια — πυκνώνει την υπάρχουσα, σε ακτές που η γεωμετρία ήδη λέει υπήνεμες.
 *
 * ΤΑΞΗ ΜΕΓΕΘΟΥΣ: 74 στις ~155.500 ώρες×παραλία = 0,05%. Είναι χειρουργική αλλαγή, όχι κανόνας.
 *
 * ΓΙΑΤΙ ΔΕΝ ΞΑΝΑΓΙΝΕ ΔΕΚΤΟ ΤΟ −0,50 ΠΟΥ ΕΙΧΕ ΑΠΟΡΡΙΦΘΕΙ. Δεν αναιρείται τίποτα από τις 16/08: το
 * −0,50 δίνει 2 καταρρεύσεις στο δάπεδο και το διπλάσιο πλάτος, και μένει απορριμμένο. Αλλά η
 * μέτρηση της 16/08 σύγκρινε ΜΟΝΟ −0,5 έναντι −0,8, σε ΜΙΑ μέρα, με προσέγγιση
 * `max(δάπεδο, min(SMB, ανοιχτά))` που ΔΕΝ περνάει από τη ράμπα — δηλαδή αυστηρότερη πτώση από
 * αυτήν που τυπώνει στ' αλήθεια η `estimateShoreWaveHeightM`. Εδώ τρέχει η ίδια η συνάρτηση.
 *
 * ⚠️ ΜΗΝ ΤΟ ΕΝΩΣΕΙΣ ΞΑΝΑ ΜΕ ΤΟ `OFFSHORE_FLAT_MAX_ONSHORE`. Εκείνο κρατάει ΤΡΙΑ πράγματα (την
 * πύλη ανέμου εδώ, το `holdsFlatWaterUnderOffshoreWind` και το `SHORE_RAMP_FULL_ONSHORE`, δηλαδή
 * την ίδια την κλίμακα της ράμπας). Κανένα από τα τρία δεν μετρήθηκε σε αυτή τη σάρωση, και
 * μετακινώντας τα μαζί θα άλλαζες τέσσερα πράγματα με ένα νούμερο.
 */
export const DEPARTING_SEA_MAX_ONSHORE = -0.65;
 ...
 *
 * ΔΥΟ ΚΑΤΩΦΛΙΑ ΑΠΟ 22/08/2026, ΓΙΑΤΙ ΕΙΝΑΙ ΔΥΟ ΕΡΩΤΗΜΑΤΑ: ο ΑΝΕΜΟΣ στο −0,80
 * (`OFFSHORE_FLAT_MAX_ONSHORE`, πάνω από 143° εκτός μετωπικής), το ΝΕΡΟ στο −0,65
 * ({@link DEPARTING_SEA_MAX_ONSHORE}, πάνω από 130°). Η αιτία και η μέτρηση των τεσσάρων
 * παραθύρων είναι γραμμένες πάνω από τη σταθερά — μην τα ξαναενώσεις χωρίς νέα μέτρηση.
 *
 * Η ΑΡΧΙΚΗ ΜΕΤΡΗΣΗ ΤΗΣ 16/08, που έδωσε το −0,8 και στα δύο
 * (scripts/measureDepartingSeaNationally.mjs, 2.768 παραλίες × 14 ώρες κολύμβησης, ζωντανά):
 *
...
export const isSeaDepartingShore = ({
  ...
  // ΤΟ ΝΕΡΟ ΚΡΙΝΕΤΑΙ ΜΕ ΔΙΚΟ ΤΟΥ ΚΑΤΩΦΛΙ (22/08/2026) — δες DEPARTING_SEA_MAX_ONSHORE παραπάνω
  // για τη μέτρηση των τεσσάρων παραθύρων. Ο άνεμος από πάνω ΔΕΝ κουνήθηκε.
  return measurable.every(
    c => typeof c.directionDeg === 'number'
      && Number.isFinite(c.directionDeg)
      && onshoreOf(c.directionDeg) <= DEPARTING_SEA_MAX_ONSHORE
  );
};
