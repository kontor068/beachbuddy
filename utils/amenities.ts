...
// ΟΙ ΛΕΞΕΙΣ ΔΕΝ ΕΙΝΑΙ ΚΟΛΛΗΤΕΣ — και ΣΗΜΕΡΑ ΔΕΝ ΑΛΛΑΖΕΙ ΤΙΠΟΤΑ ΣΤΗΝ ΟΘΟΝΗ (18/08/2026).
//
// Η προηγούμενη έκδοση ζητούσε «χωρις παροχ» ως ΣΥΝΕΧΟΜΕΝΟ κείμενο, οπότε δεν έπιανε την
// τυποποιημένη φράση της απόφασης 14/08 — «χωρίς **σταθερές** παροχές» (84 παραλίες), ούτε
// τις «χωρίς επιβεβαιωμένες σταθερές παροχές», «καμία σταθερή παροχή», «no stable services
// confirmed». 89 παραλίες δήλωναν κάτι που αυτό το φίλτρο δεν διάβαζε.
//
// ΜΕΤΡΗΘΗΚΕ ΟΜΩΣ ΕΘΝΙΚΑ ΠΡΙΝ/ΜΕΤΑ (2.872 παραλίες, πραγματική getAmenityChips): **0 αλλαγές**.
// Ο λόγος είναι ότι το ίδιο `if` ελέγχει ΚΑΙ `organized === false`, και όλες αυτές οι
// παραλίες το έχουν ήδη — δείχνουν «Χωρίς οργανωμένες παροχές» από εκείνη τη διαδρομή.
// Άρα αυτό ΔΕΝ είναι διόρθωση ορατού σφάλματος· είναι να συμφωνούν οι δύο ενδείξεις, ώστε
// μια μελλοντική παραλία με τη φράση αλλά χωρίς `organized:false` να μη σιωπήσει.
// Μην το παρουσιάσει κανείς ως κερδισμένες σελίδες — δεν είναι.
//
// ΤΙ ΜΕΝΕΙ ΕΞΩ ΕΠΙΤΗΔΕΣ. Το «χωρίς πλήρη οργάνωση» / «ελεύθερη παραλία χωρίς οργάνωση»
// (116 + 2 παραλίες) ΔΕΝ σημαίνει «τίποτα εκεί» — σημαίνει «όχι πλήρως οργανωμένη», και
// συχνά υπάρχει καντίνα. Ούτε το «χωρίς ξαπλώστρες», που αφορά ΕΝΑ είδος, όχι το σύνολο.
// Γι' αυτό το ενδιάμεσο κομμάτι είναι δεσμευμένο σε σταθερ/οργανωμεν/επιβεβαιωμεν και δεν
// είναι ελεύθερο μπάλωμα. Ελέγχθηκε 16/16 σε αληθινές φράσεις, και προς τις δύο κατευθύνσεις.
const itemSaysNoFacilities = (item: string): boolean =>
  /καμια[α-ω ]*παροχ|χωρις (σταθερ|οργανωμεν|επιβεβαιωμεν)[α-ω ]*παροχ|no beach facilit|no facilit|no stable services/.test(normalize(item));
