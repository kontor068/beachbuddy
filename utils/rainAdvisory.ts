 ...
 * rain the visitor can no longer be caught in.
 */
export const getRainSwimAdvisory = (
  ...
  const rainyHours = hourlyForecast
    .filter(isBeachHour)
    .filter(item => !isToday || item.dt * 1000 > now.getTime())
    .filter(hasHourlyRainRisk)
    .map(item => new Date(item.dt * 1000));

  if (rainyHours.length === 0) return null;

  const ranges = groupConsecutiveHours(rainyHours).slice(0, 3);
  ...
  return { timesLabel, ...buildCopy(language, timesLabel, dayPrefix, singleHour) };
};
