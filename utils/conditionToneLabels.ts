 ...
 *
 * ⚠️ 10/09/2026 — ΚΑΙ ΣΤΗΝ ΤΡΙΤΗ ΦΡΑΣΗ. Η «wind-feels-less» (24/08, Βάι) ανάβει ΜΟΝΟ στα 4-5
 * Μποφόρ με κύμα ακτής <0,40 — δηλαδή απόγειος πάνω από λάδι, η ΠΙΟ επικίνδυνη εκδοχή της ίδιας
 * συνθήκης για φουσκωτό, γιατί το νερό δείχνει ακίνδυνο. Γράφτηκε μόνο με το καθησυχαστικό μισό,
 * αντίθετα με τον κανόνα του OFFSHORE_NOTE_WARNING_MIN_BEAUFORT και το §Μ11 της βίβλου. Η φράση του
 * Μίλτου μένει αυτούσια· προστέθηκε μόνο το προειδοποιητικό μισό, ίδιες λέξεις με τη δεύτερη φράση.
 * Η πύλη validateOffshoreWindNote απαιτεί πλέον το μισό σε ΚΑΘΕ μορφή που ανάβει από 4 Μποφόρ.
 */
export const offshoreWindNoteLabels: Record<LanguageCode, Record<OffshoreWindNoteForm, string>> = {
  en: {
    ...
    'wind-feels-less': "The wind blows off the land — on the beach it will feel lighter than the figure suggests, but on a lilo or dinghy it will carry you out.",
  },
  gr: {
    ...
    'wind-feels-less': "Ο αέρας έρχεται από τη στεριά — στην παραλία θα τον νιώσεις πιο λίγο απ’ ό,τι λέει το νούμερο, αλλά με στρώμα ή φουσκωτό σε βγάζει ανοιχτά.",
  },
  fr: {
    ...
    'wind-feels-less': "Le vent souffle de la terre — sur la plage, il paraîtra plus faible que ne l’indique le chiffre, mais sur un matelas ou un canot, il vous emporte au large.",
  },
  de: {
    ...
    'wind-feels-less': "Der Wind kommt vom Land — am Strand wirst du ihn schwächer spüren, als die Zahl vermuten lässt, aber auf Luftmatratze oder Schlauchboot treibt er dich aufs offene Meer.",
  },
  it: {
    ...
    'wind-feels-less': "Il vento soffia da terra — in spiaggia lo sentirai meno di quanto dica il numero, ma su un materassino o un gommone ti porta al largo.",
  },
};
...
/**
 * «ΔΕΝ ΤΟ ΠΡΟΤΕΙΝΟΥΜΕ ΣΗΜΕΡΑ» — Η ΓΡΑΜΜΗ ΤΗΣ ΚΑΡΤΑΣ, ΟΧΙ ΤΗΣ ΛΕΖΑΝΤΑΣ (24/08/2026).
 *
 * ΤΟ ΙΣΤΟΡΙΚΟ, ΓΙΑΤΙ ΑΞΙΖΕΙ ΝΑ ΜΗΝ ΞΑΝΑΓΙΝΕΙ. Στις 23/08 η λεζάντα σταμάτησε να κρύβει τις
 * μη-προτεινόμενες πινέζες (Λέσβος: χάρτης 17 πορτοκαλί, λεζάντα «Μέτρια 1»), και η εξήγηση
 * μπήκε ΕΚΕΙ — μια δεύτερη γραμμή κάτω από κάθε χρωματικό chip. Ο Μίλτος την έκοψε αυθημερόν:
 * «πολύ κείμενο στις εξηγήσεις στα πορτοκαλί και στα κόκκινα, βγάλ' τα από εκεί, αν είναι
 * βάλ' τα μέσα στις κάρτες». Είχε δίκιο για τον λόγο που το ίδιο το αρχείο γράφει δύο φορές
 * παραπάνω: το chip της λεζάντας είναι ΟΜΑΔΙΚΟ — μιλάει για δεκαεπτά παραλίες μαζί — και μια
 * ομαδική πρόταση πάνω από τον χάρτη κοστίζει ύψος σε κάθε αναγνώστη για να απαντήσει σε
 * ερώτηση που έχει μόνο όποιος κοιτάζει ΜΙΑ παραλία.
 *
 * ΕΔΩ ΕΙΝΑΙ Η ΣΩΣΤΗ ΘΕΣΗ: η κάρτα μιλάει για μία παραλία, και ο αναγνώστης που έφτασε ως αυτήν
 * έχει ήδη διαλέξει να ρωτήσει γι' αυτήν. Η λεζάντα ξαναμετράει ό,τι βάφει ο χάρτης (αυτό
 * μένει — ήταν το πραγματικό σφάλμα) και σωπαίνει· η κάρτα εξηγεί.
 *
 * ΓΙΑΤΙ ΤΟΣΟ ΛΙΓΕΣ ΛΕΞΕΙΣ. Το «γιατί» ζει ήδη δίπλα της, σε τρία σημεία: τα δύο chip αέρα και
 * κύματος από πάνω, η ετυμηγορία στη σελίδα της παραλίας, και η πινέζα. Αυτή η γραμμή δεν
 * επαναλαμβάνει τίποτα από αυτά — απαντά μόνο στο «γιατί δεν είναι στις προτάσεις;», που
 * καμία από τις τρεις δεν απαντούσε.
 */
export const cardNotRecommendedLabels: Record<LanguageCode, string> = {
  en: 'Not recommended today',
  gr: 'Δεν το προτείνουμε σήμερα',
  fr: 'Non recommandée aujourd’hui',
  de: 'Heute nicht empfohlen',
  it: 'Oggi non consigliata',
};

/** Η φράση της κάρτας, με πτώση στα αγγλικά αν λείπει γλώσσα — ίδιο μοτίβο με τα υπόλοιπα εδώ. */
export const cardNotRecommendedLabel = (language: LanguageCode): string =>
  cardNotRecommendedLabels[language] ?? cardNotRecommendedLabels.en;
