...
export const windProfileOverridesByBeachId: Record<number, WindProfile> = {
  ...
  1817: {
    // 225° → 277° ΣΤΙΣ 20/08/2026. Το «νοτιοδυτικός όρμος» ήταν εικασία· η παραλία είναι στη
    // ΔΥΤΙΚΗ ακτή, πάνω στο στενό της Αντιπάρου. Τρεις πηγές εκτός του χειρογράφου συμφωνούν
    // ότι κοιτάει δυτικά και όχι νοτιοδυτικά: ακτογραμμή OSM 277° (μετρημένη 7 μ. από την
    // πινέζα, 22 τμήματα — `scripts/auditCoastlineFacingOsm.mjs --only 2020`, με την ομάδα
    // ελέγχου των 45 παραλιών να αναπαράγει τις γνωστές απαντήσεις σε διάμεσο 9,1°),
    // `beach.orientation` 281°, γεωμετρικό προφίλ 307°. Το χειρόγραφο απείχε 52°.
    // ΓΙΑΤΙ ΤΟ 277 ΚΑΙ ΟΧΙ ΤΟ 307: ο OSM είναι ο μόνος κριτής εκτός του σπιτιού μας, και οι
    // δύο κοντινότερες μετρήσεις (277/281) συμφωνούν μεταξύ τους. Ο κριτής σήμανε «διασπορά
    // 65°» — κόλπος με καμπύλη ακτή — άρα η ΑΚΡΙΒΗΣ μοίρα μένει προσεγγιστική· η ΠΛΕΥΡΑ όμως
    // (δυτικά, όχι νοτιοδυτικά) είναι ομόφωνη.
    // ΤΙ ΑΛΛΑΞΕ, ΜΕΤΡΗΜΕΝΟ ΑΝΑ 5° (`scripts/probeBeachExposureMatrix.mjs --beach 2020 --step 5`):
    // μόνο η ζώνη ανέμου 185°-200°, όπου η πινέζα έβγαινε ΚΟΚΚΙΝΗ ενώ η κάρτα έλεγε
    // «Προστατευμένη» και η γεωμετρία onshore −0,60 (ο αέρας φεύγει από τη στεριά). Τώρα
    // συμφωνούν. Στα κέντρα των οκτώ τομέων δεν άλλαξε ΤΙΠΟΤΑ — γι' αυτό μετρήθηκε ανά 5°.
    beachFacingDirection: 277,
    ...
    // (facing 225), αλλά η ακτογραμμή κοιτάει δυτικά και ο ΒΔ τομέας μετριέται fetch 10,08 χλμ /
